...
База: PRD v0.2 (`docs/product-ux/PRD_Экостройконтинент_v0.2.md`)
...
- Каноническая launch truth для phase 1: RU-first.
- Английский допускается архитектурно, но не обещается как launch-equal locale без подтверждённой editorial capacity команды.
...
- Нужно ли поднимать EN после RU-first запуска и кто владеет регулярным сопровождением этой локали.
...
PRD v0.3 — рабочий канон. Более глубокие product- или implementation-спеки создаются отдельно только там, где появляется реальная потребность.
