...
### 10.2 Media library bulk mark

Экран `Медиа` использует существующий групповой выбор карточек как дополнительный вход в тот же removal quarantine workflow:

- при выборе нескольких карточек в панели групповых действий появляется компактная иконка корзины;
- иконка является семантической кнопкой с доступным именем `Пометить выбранные на удаление`;
- действие всегда требует отдельного подтверждения и не выполняет hard delete;
- подтверждение показывает общее число выбранных карточек, число уже помеченных карточек и число карточек, скрытых текущим фильтром;
- уже помеченные карточки обрабатываются как idempotent no-op;
- частичный результат обязан явно разделять успешно помеченные, уже помеченные и не обработанные карточки;
- после успешной пометки UI предлагает перейти в существующий `Центр очистки`;
- bulk action не получает права запускать sweep, снимать публикацию или удалять бинарные файлы.

Групповой и одиночный входы обязаны использовать один канонический `mark + audit` command. UI не должен реализовывать removal policy или вызывать purge по набору карточек.

### 10.3 Entity registry / lists
...
- отдельная вкладка показывает карточки, которые пока нельзя удалить, с одной главной причиной обычным языком и, если роль может открыть entity editor, ссылкой на точный блокирующий объект;
...
- каждая ready-карточка имеет отдельную иконку корзины, которая запускает тот же server preview и confirm для одной removal component, а не отдельный direct-delete path;
...
Матрица доступа к этому экрану является отдельной от content editing и generic maintenance:

- `superadmin`, `seo_manager` и `business_owner` могут просматривать очередь и причины блокировки;
- `superadmin` и `business_owner` могут подтверждать одиночную или групповую очистку ready-компонентов;
- `seo_manager` остаётся read-only на экране очистки;
- доступ владельца бизнеса к очистке не даёт ему content edit, publish/unpublish, mark/unmark, user management или generic maintenance purge;
- если роль не имеет доступа к entity editor или private media preview, экран не должен показывать ей неработающие ссылки и закрытые thumbnails.
