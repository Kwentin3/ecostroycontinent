...
- Admin Console is the write-side tool for editorial operations.
- Public Web is a published read-side projection and must not become owner of editorial truth.
...
- Lead and intake records stay in a neighboring operational domain and are not authored as content entities.
...
- Purpose: standalone public page or read-side page shell, assembled from typed blocks and relations
...
`Page` should not become a second owner of canonical route truth for `Service`, `Case` or `Article`.

### 4.2 Service

- Purpose: canonical route-owning service entity for money pages and reuse in blocks/articles/FAQs
...
- Purpose: canonical route-owning project/case entity for proof-led launch content
...
- Purpose: canonical route-owning entity for the minimal supporting knowledge layer
...
- published page content can be rendered publicly as the current read-side projection
