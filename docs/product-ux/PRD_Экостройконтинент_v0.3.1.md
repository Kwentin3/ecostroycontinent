...
- Hero + CTA.
- Услуги: компактные кликабельные карточки с коротким описанием и published-only медиалентой; Home service hub и `/services` используют один общий визуальный контракт.
- Кейсы / портфолио.
- FAQ.
- Отзывы.
...
## 12. Addendum - Public Launch Domain Clarifications (2026-04-17)

Этот addendum является нормативным уточнением для phase 1 и дополняет разделы 3, 4 и 9 без переписывания PRD с нуля.

### 12.1 Public launch model

- Public launch не равен одному универсальному лендингу.
- Day-1 launch-core строится на связке: `Home -> Services Index -> Service Detail -> Case/Proof -> Contact Action`.
- Обязательный day-1 route core: `/`, `/services`, `/services/[slug]`, `/cases`, `/cases/[slug]`, `/about`, `/contacts`.
- `/blog` и `Article` остаются phase-1 supporting layer и включаются в live после готовности route/entity/publish contour.

### 12.2 Route ownership hard boundary

- `Service`, `Case`, `Article` остаются route-owning entities для своих URL.
- `Page` владеет standalone pages и page-level composition only.
- Любая формулировка про page-managed service/equipment route не должна трактоваться как право `Page` дублировать или подменять route truth `Service`/`Case`/`Article`.

### 12.3 Navigation as architecture

Навигация входит в launch-canonical model как обязательная система:
...
### 12.4 Publish and SEO coupling

Publish и public SEO delivery считаются связанной системой:

- publish gates обязательны для indexable surfaces
- slug mutation published entities требует redirect/revalidation/sitemap follow-up
- запуск невозможен без минимального technical SEO baseline (`robots`, `sitemap`, canonical/meta/indexation/schema where factual)

### 12.5 Companion doc

Операционный канон phase-1 public domain зафиксирован в companion документе:

- `docs/product-ux/Public_Launch_Domain_Canon_Экостройконтинент_v0.1.md`

### 12.6 Service scope presentation profile

Day-1 допускает узкий временный профиль отображения для блока `Что входит в услугу` на системной странице `/services/[slug]`.

Текущий честный контракт:

- `Service.serviceScopeDisplayMode` может принимать только `default` или `columns`.
- `default` сохраняет обычный исправленный plaintext-rendering с абзацами и списками.
- `columns` применяется только к `Service.serviceScope` на публичной странице услуги и раскладывает абзацы в аккуратные колонки.
- Если текст блока содержит распознанный нумерованный список, runtime обязан fallback'нуться в `default`, чтобы не ломать смысл последовательности.
- Поле является временной presentation hint в `Service`, а не новой владельческой моделью страницы.

Future contract:

- После рефакторинга экрана страницы и логики сборки service detail секций выбор профиля должен переехать в page-owned слой.
- Переключатель в карточке услуги и переключатель на экране страницы должны показывать один и тот же актуальный профиль без расхождения.
- До этого рефакторинга экран страницы не должен притворяться владельцем этой настройки.
