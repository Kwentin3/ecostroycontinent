...
| Publish approved revision | Yes | No | No |
| Publish revision not requiring owner review | Yes | No | No |
...
- `Approve`: revision получает owner-approved marker и может быть опубликована Superadmin, если readiness gates пройдены.
...
| `Page(type=about)` | Minor editorial | Yes by default | `Superadmin` after approval |
| `Page(type=about)` | SEO-only operational | Yes by default | `Superadmin` after approval |
| `Page(type=about)` | Commercial presentation | Yes | `Superadmin` after approval |
| `Page(type=about)` | Route-affecting | Yes | `Superadmin` after approval |
| `Page(type=contacts)` | Minor editorial after confirmed contact truth | No by default | `Superadmin` |
| `Page(type=contacts)` | SEO-only operational after confirmed contact truth | No by default | `Superadmin` |
| `Page(type=contacts)` | Global truth change | Yes | `Superadmin` after approval |
| `Page(type=contacts)` | First publish before confirmed contact truth | Publish blocked | No publish allowed |
| `Page(type=contacts)` | First publish after confirmed contact truth | Yes | `Superadmin` after approval |
