...
- давать SEO Manager рабочую среду для подготовки контента, SEO-полей и публикации approved review revisions;
- давать Business Owner отдельную owner review surface для быстрых и понятных решений;
- давать Superadmin контроль над exception publish, rollback, пользователями и audit trails;
...
Superadmin отвечает за operational integrity системы. Это не просто “самый главный редактор”, а role-holder, который контролирует пользователей, exception publish operations, rollback и forensic visibility. В phase 1 Superadmin остается operational authority, но не единственным publisher: SEO Manager может публиковать approved review revisions.

#### SEO Manager

SEO Manager работает в editor / content operations mode. Он создает и нормализует сущности, заполняет SEO-поля, связывает контент и media, запускает AI-assistive actions, подготавливает материалы к review, устраняет readiness blockers и публикует approved review revisions.
...
| Publish approved revision | Yes | Yes, for supported Content Core resources after required approval | No |
| Publish revision not requiring owner review | Yes | No; SEO publish requires explicit review approval | No |
...
| --- | --- | --- | --- |
| `Service` | Minor editorial | No by default | `Superadmin`; `SEO Manager` only after explicit review approval |
| `Service` | SEO-only operational | No by default | `Superadmin`; `SEO Manager` only after explicit review approval |
| `Service` | Commercial presentation | Yes for launch-core services | `SEO Manager` or `Superadmin` after approval |
| `Service` | Route-affecting | Yes | `SEO Manager` or `Superadmin` after approval |
| `Service` | First publish | Yes | `SEO Manager` or `Superadmin` after approval |
| `Case` | Minor editorial | No by default for non-flagship routine proof updates | `Superadmin`; `SEO Manager` only after explicit review approval |
| `Case` | SEO-only operational | No by default for non-flagship case | `Superadmin`; `SEO Manager` only after explicit review approval |
| `Case` | Commercial presentation | Yes for flagship or claims-heavy case | `SEO Manager` or `Superadmin` after approval |
| `Case` | Route-affecting | Yes | `SEO Manager` or `Superadmin` after approval |
| `Case` | First publish | Yes for priority or flagship case | `SEO Manager` or `Superadmin` after approval |
...
| `Page(type=contacts)` | Minor editorial after confirmed contact truth | No by default | `Superadmin`; `SEO Manager` only after explicit review approval |
| `Page(type=contacts)` | SEO-only operational after confirmed contact truth | No by default | `Superadmin`; `SEO Manager` only after explicit review approval |
...
| `Global Settings` | Any substantive truth change | Yes | `SEO Manager` or `Superadmin` after approval |
...
Publish authority: Superadmin; SEO Manager only if the revision has explicit review approval and passes readiness.
...
Publish authority: SEO Manager or Superadmin after approval and readiness.
...
Publish authority: SEO Manager or Superadmin after approval, readiness and required side effects.
...
Перед publish разрешенный publisher видит отдельную readiness surface:
...
- SEO Manager может создать draft revision, заполнить SEO fields, связать entities и media, увидеть readiness blockers, отправить revision на review и опубликовать approved review revision.
...
- Superadmin может опубликовать validated revision, увидеть publish checks, выполнить rollback и управлять пользователями fixed-role model.
