...
Page workspace в текущем виде уже не выглядит вторым параллельным editor flow: домен `Страницы`, registry-first вход, единый workspace, metadata modal и bounded AI действительно собраны в одну модель. На страницах, у которых уже есть draft/published revision, базовый ежедневный сценарий SEO-оператора работает: открыть страницу, поправить тексты, сохранить, посмотреть preview, открыть metadata и передать страницу в review.
...
Theme control реализован и находится в metadata modal (`Основное`).

Проверено:

- theme selector переключается;
- save metadata проходит;
- header badge with theme label меняется;
