...
export function getPublicNavItems({ includeCases = true } = {}) {
  if (includeCases) {
    return PUBLIC_NAV_ITEMS;
  }

  return PUBLIC_NAV_ITEMS.filter((item) => item.key !== "cases");
}
...
export function shouldRenderServiceQuickAccess(pathname, quickLinks = []) {
  ...
  return normalized.startsWith("/services/") || normalized.startsWith("/cases/");
}
