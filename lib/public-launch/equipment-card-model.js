...
function resolveActions({ ctaAction, ctaActions, ctaLabel }) {
  const rawActions = Array.isArray(ctaActions) && ctaActions.length > 0
    ? ctaActions
    : [ctaAction];
  const actions = rawActions
    .map((action, index) => resolveAction({
      ctaAction: action,
      ctaLabel: index === 0 ? ctaLabel : ""
    }))
    .filter(Boolean);

  return uniqueBy(actions, (action) => `${action.href}|${action.label}`);
}
...
export function buildEquipmentCardModel({
  ...
  const actions = resolveActions({ ctaAction, ctaActions, ctaLabel });
