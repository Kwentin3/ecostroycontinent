...
function buildFallbackModeState(reason = "режим начальной загрузки по умолчанию") {
  return {
  ...
  return withTransaction(async (db) => {
    ...
    const insertResult = await queryWithDb(
      ...
      [
        ...
        actorUserId ?? null,
        DISPLAY_MODE_AUDIT_EVENT_KEY,
        `Режим отображения переключен на ${mode}`,
        JSON.stringify({
...
export async function ensureDisplayModeState(db = null) {
  await queryWithDb(
    ...
    [DEFAULT_PUBLIC_DISPLAY_MODE, "режим начальной загрузки по умолчанию"]
  );
}
...
export async function setDisplayModeState({
  ...
  if (!parsedMode) {
    throw new Error(`Неподдерживаемый режим отображения: ${String(mode)}`);
  }
