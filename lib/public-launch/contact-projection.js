...
function resolveReadiness({ truthConfirmed, phone, serviceArea, email }) {
  const hasPublicContact = Boolean(phone || email);

  if (truthConfirmed && hasPublicContact && serviceArea) {
    return {
      code: "ready",
...
function buildPublicContactItems({ truthConfirmed, phone, phoneHref, email, emailHref }) {
  if (!truthConfirmed) {
    return [];
  }
...
export function buildPublicContactProjection(globalSettings, { currentPath = "/" } = {}) {
  ...
  const publicContactItems = buildPublicContactItems({
    truthConfirmed,
    phone,
    phoneHref,
    email,
