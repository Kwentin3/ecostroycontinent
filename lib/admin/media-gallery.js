...
import { getLivePublicationStatusModel, getWorkingRevisionStatusModel } from "./workflow-status.js";
...
function buildAdminMediaPreviewUrl({ entityId, revision }) {
  if (!entityId) {
    return "";
  }

  const versionKey = revision?.id || revision?.updatedAt || "";
  const version = versionKey ? `?v=${encodeURIComponent(versionKey)}` : "";

  return `/api/admin/media/${entityId}/preview${version}`;
}

export function getAdminMediaPreviewUrls({ entityId, storageKey, latestRevision, publishedRevision, config } = {}) {
  if (!storageKey) {
    return {
      previewUrl: "",
      deliveryUrl: "",
  ...
  const previewUrl = buildAdminMediaPreviewUrl({ entityId, revision: latestRevision });
  const deliveryUrl = getMediaDeliveryUrl({ entityId, storageKey }, config) || "";
  const currentRevisionIsPublished = Boolean(
    deliveryUrl
    && latestRevision?.state === "published"
    && latestRevision?.id
  ...
  return {
    previewUrl,
    deliveryUrl,
    thumbnailUrl: currentRevisionIsPublished ? deliveryUrl : previewUrl
  };
}
...
function buildMediaCardDto({ card, publishedRevision, usageEntries, collectionEntries, binaryHealthy }) {
  ...
  const liveStatus = getLivePublicationStatusModel({
    currentRevision: latestRevision,
    activePublishedRevision: publishedRevision
  });
  ...
  const previewUrls = getAdminMediaPreviewUrls({
    entityId: card.entity.id,
  ...
  return {
    ...
    reviewComment: latestRevision?.reviewComment ?? "",
    ...
    deliveryUrl: previewUrls.deliveryUrl,
    thumbnailUrl: previewUrls.thumbnailUrl,
    statusKey,
    statusLabel: workingStatus.label,
    statusTone: workingStatus.tone,
    workingStatusKey: workingStatus.key,
    workingStatusDescription: workingStatus.description,
    liveStatusLabel: liveStatus.label,
    liveStatusTone: liveStatus.tone,
    liveStatusDescription: liveStatus.description,
...
function buildCollectionCardDto({ card, publishedRevision = null, usageEntries }) {
  const latestRevision = card.latestRevision;
  const payload = latestRevision?.payload ?? {};
  const usage = deriveUsageSummary(usageEntries);
  const statusKey = latestRevision?.state ?? "draft";
  ...
  return {
    ...
    ownerReviewRequired: Boolean(latestRevision?.ownerReviewRequired),
    ownerApprovalStatus: latestRevision?.ownerApprovalStatus ?? "not_required",
    previewStatus: latestRevision?.previewStatus ?? "preview_renderable",
    publishedRevisionNumber: publishedRevision?.revisionNumber ?? null,
    markedForRemovalAt: card.entity.markedForRemovalAt ?? null,
    title: labelFromPayload(payload),
    caption: payload.caption || "",
...
async function buildPublishedCollectionMap() {
  const publishedCards = await listPublishedCards(ENTITY_TYPES.GALLERY);
  return new Map(publishedCards.map((card) => [card.entityId, card.revision]));
}
...
export async function listCollectionLibraryCards() {
  ...
  const publishedMap = await buildPublishedCollectionMap();
...
export async function getCollectionLibraryCard(entityId) {
  ...
  return buildCollectionCardDto({
    ...
    publishedRevision: state.activePublishedRevision ?? null,
    usageEntries: usageIndex.get(entityId) ?? []
  });
}
