...
const MEDIA_USAGE_REFERENCE_TYPES = [
...
const COLLECTION_USAGE_REFERENCE_TYPES = [
  ENTITY_TYPES.SERVICE,
  ENTITY_TYPES.CASE,
  ENTITY_TYPES.PAGE
];
        ...
        for (const galleryId of block.galleryIds ?? []) {
          pushUsageEntry(
            ...
            createReferenceEntry({
              ...
              entityId,
              revision,
              relationLabel: block.title ? `Блок страницы: ${block.title}` : "Галерейный блок страницы",
              statusKey
            })
          );
        }
...
async function buildMediaUsageIndex() {
  return buildReferenceIndex(MEDIA_USAGE_REFERENCE_TYPES, collectMediaUsageFromRevision);
}

async function buildCollectionUsageIndex() {
  return buildReferenceIndex(COLLECTION_USAGE_REFERENCE_TYPES, collectCollectionUsageFromRevision);
}
