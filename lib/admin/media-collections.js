...
export function buildCollectionDraftInputWithAssetMembership({ currentPayload = {}, assetId, includeAsset }) {
  const normalizedAssetId = normalizeString(assetId);
  const currentAssetIds = normalizeIdArray(currentPayload.assetIds ?? []);
  const nextAssetIds = includeAsset
    ? normalizeIdArray([...currentAssetIds, normalizedAssetId])
    : currentAssetIds.filter((value) => value !== normalizedAssetId);
  const currentPrimaryAssetId = normalizeString(currentPayload.primaryAssetId);
  const nextPrimaryAssetId = nextAssetIds.includes(currentPrimaryAssetId) ? currentPrimaryAssetId : "";

  return buildCollectionDraftInput({
    fields: {
      title: normalizeString(currentPayload.title),
      caption: normalizeString(currentPayload.caption),
      assetIds: nextAssetIds,
      primaryAssetId: nextPrimaryAssetId
    },
    currentPayload,
    fallbackTitle: normalizeString(currentPayload.title) || "Коллекция"
  });
}
