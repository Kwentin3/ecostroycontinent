...
export const MEDIA_COLLECTION_CANDIDATE_FILTERS = Object.freeze([
  { key: "all", label: "Все" },
  { key: "ready-to-publish", label: "К публикации" },
  { key: "published", label: "Опубликовано" }
]);
...
export function assetHasPublishedRevision(item = {}) {
  return Boolean(item?.publishedRevisionNumber || item?.statusKey === "published");
}
...
export function matchesMediaCollectionCandidateFilter(item = {}, filterKey = "all") {
  switch (filterKey) {
    case "ready-to-publish":
      return mediaAssetReadyToPublish(item);
    case "published":
      return assetHasPublishedRevision(item);
    default:
      return true;
  }
}
