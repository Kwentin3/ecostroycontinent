  ...
  for (const sourceType of sourceTypes) {
    for (const published of catalog.publishedByType[sourceType] ?? []) {
      ...
      if (sourceType === ENTITY_TYPES.GALLERY) {
        blockers.push(makeGalleryReason());
        continue;
      }
...
export async function evaluateTestGraphTeardown(input, deps = {}) {
  ...
  while (queue.length > 0) {
    ...
    if (!member.isTestData) {
      blockers.push("В графе есть route-owning сущность без test marker.");
      continue;
    }

    if (member.hasReviewRevision) {
      blockers.push("Один из объектов графа участвует в review/publish-потоке и не может быть разобран автоматически.");
    }
    ...
    for (const ref of outgoingRefs) {
      ...
      if (!isAgentTestCreationOrigin(targetAggregate.entity.creationOrigin)) {
        blockers.push(makeOutgoingMixedReason(ref.entityType));
        continue;
      }
