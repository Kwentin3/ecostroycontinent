...
const STATE_NOTES = {
  blocked: "Сначала откройте проблемные поля и исправьте блокирующие замечания.",
  partial: "Проверка уже есть, но часть слоя данных ещё не в порядке.",
  ready: "По текущей проверке карточка не содержит блокирующих проблем.",
...
function buildPrimaryAction({ entityType, problemItems, fallbackAnchorId, fallbackLabel }) {
  ...
  if (nextProblem) {
    return {
      ...
      note: nextProblem.message || nextProblem.reason || "Откройте указанное поле и снимите блокирующее замечание.",
  ...
  if (starterAnchor) {
    return {
      ...
      note: "Это первый точный якорь в данных сущности.",
      isFallback: false,
  ...
  return {
    ...
    note: "Точного якоря пока нет, поэтому открывается резервный раздел.",
    isFallback: true,
