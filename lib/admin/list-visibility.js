...
export function buildListRowProjection({
  ...
  const signalMeta = getSignalMeta(badge.state);
  const actionHref = appendAdminReturnTo(`/admin/entities/${entityType}/${card.entity.id}`, listHref);
  const reason = localizeReason(badge.reason);
  ...
  return {
    ...
    versionStateLabel: getRevisionStateLabel(card.latestRevision.state),
    signalState: badge.state,
    signalLabel: signalMeta.label,
    signalTone: signalMeta.tone,
    signalReason: reason,
    ...
    actionHref,
    actionLabel: signalMeta.actionLabel,
    priority: SIGNAL_PRIORITY[badge.state] ?? SIGNAL_PRIORITY.neutral,
...
export function buildListSurfaceViewModel(rows = []) {
  ...
  const summaryNote =
    summary.blocked > 0 || summary.proof_gap > 0
      ? "Сначала открывайте blocked и proof gap rows; сигнал уже виден прямо в списке."
      : summary.partial > 0 || summary.missing > 0
        ? "Список выглядит неполным: подробности откройте в detail view."
        : "Сигналы показаны прямо в строках; healthy rows можно открывать последними.";
