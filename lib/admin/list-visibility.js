...
import { getWorkingRevisionStatusModel } from "./workflow-status.js";
...
const SIGNAL_REASON_COPY = Object.freeze({
  "No blocking gaps projected.": "Блокирующих пробелов не видно.",
...
function getPrimaryMediaPreviewUrl(entityType, payload = {}) {
  const mediaId = typeof payload?.primaryMediaAssetId === "string" ? payload.primaryMediaAssetId.trim() : "";

  if (!mediaId) {
    return "";
  }

  if ([ENTITY_TYPES.SERVICE, ENTITY_TYPES.EQUIPMENT, ENTITY_TYPES.CASE].includes(entityType)) {
    return `/api/admin/media/${mediaId}/preview`;
  }
...
function buildMissingVersionRow({ card, entityType, listHref }) {
  ...
  return {
    ...
    isMarkedForRemoval: Boolean(card.entity.markedForRemovalAt),
...
export function buildListRowProjection({
  ...
  const reason = inactiveLiveTruth
    ? "Опубликованная версия сохранена в истории, но активная публикационная привязка уже снята."
    : localizeReason(badge.reason);
  const previewMediaUrl = getPrimaryMediaPreviewUrl(entityType, card.latestRevision.payload);
  const workingStatus = getWorkingRevisionStatusModel({
    currentRevision: card.latestRevision,
    activePublishedRevision: card.entity.activePublishedRevisionId
      ? { id: card.entity.activePublishedRevisionId }
      : null
  });
  ...
  return {
    ...
    versionStateLabel: inactiveLiveTruth ? "Снято с публикации" : workingStatus.label,
...
export function buildListSurfaceViewModel(rows = []) {
  ...
  const bullets = [
    ...
    `Снято с публикации: ${summary.inactive}`
  ];
