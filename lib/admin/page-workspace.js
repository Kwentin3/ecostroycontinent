...
export const PAGE_TYPE_LABELS = Object.freeze({
  [PAGE_TYPES.HOME]: "Главная",
  [PAGE_TYPES.ABOUT]: "О нас",
  [PAGE_TYPES.CONTACTS]: "Контакты",
  [PAGE_TYPES.SERVICE_LANDING]: "Страница услуги (устаревшая, не для запуска)",
  [PAGE_TYPES.EQUIPMENT_LANDING]: "Страница техники (устаревшая, не для запуска)"
});
...
export const PAGE_SECTION_LABELS = Object.freeze({
  ...
  [PAGE_SECTION_TYPES.SERVICE_LIST]: "Услуги",
  ...
  switch (section.type) {
    ...
    case PAGE_SECTION_TYPES.SERVICE_LIST:
      return {
        ...
        title: section.title || "",
        body: section.body || "",
        serviceIds: toList(section.serviceIds)
      };
    ...
    switch (section.type) {
      ...
      case PAGE_SECTION_TYPES.SERVICE_LIST:
        return normalizeSection({
          ...section,
          order: index,
          title: serviceListBlock?.title || section.title,
          body: serviceListBlock?.body || section.body,
          serviceIds: Array.isArray(serviceListBlock?.serviceIds) ? serviceListBlock.serviceIds.filter(Boolean) : section.serviceIds
        }, index);
      case PAGE_SECTION_TYPES.SERVICE_SCOPE:
        return normalizeSection({
          ...section,
    ...
    switch (type) {
      case PAGE_SECTION_TYPES.SERVICE_LIST:
        return {
          type,
          order: index,
          title: pageType === PAGE_TYPES.HOME ? "" : PAGE_SECTION_LABELS[type],
          body: "",
          serviceIds: []
        };
      case PAGE_SECTION_TYPES.PROOF_CASES:
...
export function buildPageWorkspaceBaseValue(revision = null) {
  ...
  const sourceRefs = toSourceRefs({
    ...
    serviceIds: payload?.sourceRefs?.serviceIds || serviceListBlock?.serviceIds || [],
  ...
  return {
    pageType,
    slug: payload?.slug || (pageType === PAGE_TYPES.HOME ? "home" : pageType === PAGE_TYPES.CONTACTS ? "contacts" : pageType === PAGE_TYPES.ABOUT ? "about" : ""),
    pageThemeKey: payload?.pageThemeKey || DEFAULT_LANDING_PAGE_THEME_KEY,
    title: payload?.title || "",
    h1: payload?.h1 || "",
    intro: payload?.intro || "",
    primaryMediaAssetId: payload?.primaryMediaAssetId || "",
    mediaSettings: normalizePageMediaSettings(payload?.mediaSettings, pageType),
    sourceRefs,
    targeting,
    sections,
    ...
    serviceIds: sourceRefs.serviceIds?.length ? sourceRefs.serviceIds : (sourceRefs.primaryServiceId ? [sourceRefs.primaryServiceId] : []),
...
export function buildPageWorkspaceCompositionState(value = {}) {
  ...
  return {
    ...
    mediaSettings: normalizePageMediaSettings(value.mediaSettings, pageType),
    sourceRefs: toSourceRefs(value.sourceRefs),
    targeting: toTargeting(value.targeting),
    sections: readSectionPayloads(value, pageType),
    body: value.body || "",
...
export function buildPageWorkspaceMetadataState(value = {}) {
  ...
  return {
    slug: value.slug || (pageType === PAGE_TYPES.HOME ? "home" : pageType === PAGE_TYPES.CONTACTS ? "contacts" : pageType === PAGE_TYPES.ABOUT ? "about" : ""),
    pageType,
  ...
  const nextSections = normalizeSectionsForType(nextMetadata.pageType, nextComposition.sections).map((section) => {
    switch (section.type) {
      ...
      case PAGE_SECTION_TYPES.SERVICE_LIST:
        return {
          ...section,
          serviceIds: nextComposition.sourceRefs.serviceIds
        };
      case PAGE_SECTION_TYPES.EQUIPMENT_SUMMARY:
        return {
          ...section,
...
export function buildPageWorkspaceLifecycleState({ aggregate = null, permissions = {} } = {}) {
  ...
  return {
    ...
    hasPublishedHistory,
    hasReviewRevision,
    canUnpublish: Boolean(permissions.canUnpublish) && hasLivePublishedRevision,
  ...
  switch (aiAction) {
    case "strengthen_cta":
      return {
        ...
        hint: "ИИ предлагает только текстовую правку выбранного блока.",
        progressLabel: "ИИ готовит новую формулировку призыва."
      };
    case "compact_wording":
      return {
        ...
        hint: "ИИ не меняет маршруты, источники и метаданные.",
        progressLabel: "ИИ сокращает текст страницы."
      };
    case "suggest_connective_copy":
      return {
        ...
        hint: "ИИ работает только с серединой страницы.",
        progressLabel: "ИИ подбирает связующий текст."
      };
    ...
    default:
      return {
        ...
        hint: "ИИ вернет только ограниченный патч для явного применения.",
        progressLabel: "ИИ переписывает выбранную зону."
      };
  }
}
