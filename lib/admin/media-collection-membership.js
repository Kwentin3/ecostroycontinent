...
export async function syncAssetCollectionMembership({
  ...
  const collectionCards = await listEntityCards(ENTITY_TYPES.GALLERY);
  ...
  for (const collectionId of touchedCollectionIds) {
    const state = await getEntityEditorState(collectionId);
  ...
  const item = await getMediaLibraryCard(normalizedAssetId);
