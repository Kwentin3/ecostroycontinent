...
// Owner-facing projection: keep the journal human-readable and omit raw
// revision ids, fingerprints, payload fragments and other engineering details.
const REVIEW_JOURNAL_PRESENTATION = Object.freeze({
  ...
  [AUDIT_EVENT_KEYS.PUBLISHED]: {
    actionLabel: "Опубликовано",
    actionText: "опубликовал",
    tone: "success"
  }
});
...
function pickEventTitle(event = {}) {
  return excerpt(event.details?.materialTitle || "", 96) || pickTitle(event.revisionPayload || {});
}
