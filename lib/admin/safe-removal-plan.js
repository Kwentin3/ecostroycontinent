import { ENTITY_TYPE_LABELS } from "../content-core/content-types.js";
...
function getEntityTypeLowerLabel(entityType) {
  return (ENTITY_TYPE_LABELS[entityType] || "сущность").toLowerCase();
}
...
function buildManualRefAction(ref, currentHref) {
  ...
  const entityLabel = getEntityTypeLowerLabel(ref.entityType);
...
function buildPublishedRefStep(publishedIncomingRefs, currentHref) {
  ...
  return makeCurrentStep(
    "published-incoming",
    "Освободить объект из опубликованных связей",
    `Этот объект всё ещё используется в опубликованной ${getEntityTypeLowerLabel(firstRef.entityType)} «${firstRef.label}». Сначала уберите связь и перепубликуйте связанный объект.`,
...
function buildDraftRefStep(publishedIncomingRefs, draftIncomingRefs, currentHref) {
  ...
  const description = `В рабочем черновике ${getEntityTypeLowerLabel(firstRef.entityType)} «${firstRef.label}» всё ещё есть ссылка на этот объект. Уберите связь и сохраните черновик.`;
...
function buildTestGraphPlan({
  ...
  if (blockingRefs.length === 0) {
    ...
    steps.push(makeCurrentStep(
      "test-graph-links",
      "Освободить тестовый граф от живых зависимостей",
      `Тестовый граф всё ещё удерживается ${getEntityTypeLowerLabel(firstRef.entityType)} «${firstRef.label}». Сначала уберите эту связь или выведите связанный объект из live-контура.`,
      buildManualRefAction(firstRef, currentHref)
    ));
  }
