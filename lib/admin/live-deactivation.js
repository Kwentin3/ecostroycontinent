...
function makePublishedRefReason(sourceType) {
  return `На сущность ссылается опубликованная ${ENTITY_TYPE_LABELS[sourceType]?.toLowerCase() || "сущность"}.`;
}

function makeDraftRefReason(sourceType) {
  return `На сущность ссылается рабочий нетестовый черновик ${ENTITY_TYPE_LABELS[sourceType]?.toLowerCase() || "сущности"}.`;
}
...
function buildIncomingRefItem(sourceType, card, state) {
  ...
  return {
    ...
    entityId: card?.entity?.id ?? card?.entityId ?? "",
    label: labelFromPayload(payload, sourceType, card?.entity?.id ?? card?.entityId ?? ""),
    href: sourceType === ENTITY_TYPES.GALLERY
      ? `/admin/entities/media_asset?compose=collections&collection=${card?.entity?.id ?? card?.entityId ?? ""}`
      : `/admin/entities/${sourceType}/${card?.entity?.id ?? card?.entityId ?? ""}`,
    state,
