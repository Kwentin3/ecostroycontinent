...
export function getReviewWorkflowStatusKey(revision = {}) {
  ...
  // "not_required" means the revision already cleared the review lane and now
  // waits only for an explicit publish action on the source entity screen.
  if (revision.ownerApprovalStatus === "approved" || revision.ownerApprovalStatus === "not_required") {
    return "approved";
  }
