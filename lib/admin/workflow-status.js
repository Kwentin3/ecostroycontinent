...
// Admin content state is projected on two independent axes:
// 1. the working revision workflow state;
// 2. the live publication state.
// Keep them separate. "Approved" / "ready to publish" is not the same as
// "already live", and "published" is not an edit lock.
export function getWorkingRevisionStatusModel({
  ...
  if (currentRevision.state === "review") {
    if (currentRevision.ownerApprovalStatus === "approved") {
      return {
    ...
    return {
      key: "internal_review",
      label: "Внутренняя проверка",
      tone: "warning",
      description: "Версия находится во внутренней проверке без решения собственника."
    };
  }
