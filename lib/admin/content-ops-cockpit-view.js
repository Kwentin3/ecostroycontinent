...
function buildStateNote(summary) {
  if ((summary?.blocked ?? 0) > 0) {
    return "Есть блокирующие gaps. Начните с следующего действия и не считайте покрытие healthy.";
  }

  if ((summary?.missing ?? 0) > 0 || (summary?.needsProof ?? 0) > 0 || (summary?.partial ?? 0) > 0) {
    return "Покрытие неполное. Пустые и частичные состояния показаны честно, без маскировки под healthy.";
  }

  if ((summary?.ready ?? 0) > 0) {
    return "Launch-core покрыт. Продолжайте с ближайшей карточки.";
  }

  return "Покрытие ещё не собрано. Начните с global settings.";
}
  ...
  return [
    {
      ...
      tone: "danger",
      description: "Сначала исправляем блокирующие gaps."
    },
    {
      key: "missing",
      label: "Не заведено",
    ...
    },
    {
      ...
      tone: "warning",
      description: "Есть контент, но не хватает proof/evidence."
    },
    {
      key: "ready",
      label: "Готово",
...
function toActionItem(row) {
  ...
  return {
    ...
    reason: row.badge?.reason || row.evidence?.summary || "Projection is incomplete.",
    actionLabel: getActionVerb(row.status),
    routeTarget,
    routeHint: routeTarget.isFallback ? `Fallback route: ${routeTarget.label}` : routeTarget.label,
...
function buildCoverageSummary(group) {
  ...
  if (group.status === "blocked") {
    return `${group.blockedCount} блокирующих gaps из ${group.total}.`;
  }
  ...
  return `${group.readyCount}/${group.total} ready.`;
}
...
export function buildCockpitSurfaceViewModel(cockpitProjection = {}) {
  ...
  if (nextActions.length === 0) {
    nextActions.push({
      ...
      routeHint: "Fallback route: Настроить глобальные настройки",
      fallbackRoute: true
    });
  }
  ...
  return {
    ...
    coverageNote:
      summary.missing > 0 || summary.partial > 0 || summary.blocked > 0 || summary.needsProof > 0
        ? "Пустые и частичные states показаны честно. Ни одно missing покрытие не считается healthy."
        : "Coverage не скрывает gaps и не пересчитывает truth."
  };
}
