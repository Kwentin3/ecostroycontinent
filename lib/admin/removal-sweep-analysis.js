...
import { getAdminMediaPreviewUrls } from "./media-gallery.js";
  ...
  for (const sourceType of SOURCE_ENTITY_TYPES) {
    for (const publishedCard of publishedByType[sourceType] ?? []) {
      ...
      for (const member of componentMembers) {
        ...
        const sourceLabel = getCardLabel(publishedCard);
        ...
        publishedIncomingRefs.push(
          makeIncomingRefItem(
            ...
            publishedCard,
            "published",
            `Используется в опубликованном объекте «${sourceLabel}».`
          )
        );
      }
    }
    ...
    for (const latestCard of latestByType[sourceType] ?? []) {
      ...
      for (const member of componentMembers) {
        ...
        const sourceLabel = getCardLabel(latestCard);
        ...
        draftIncomingRefs.push(
          makeIncomingRefItem(
            ...
            latestCard,
            "draft",
            `Используется в рабочем черновике «${sourceLabel}».`
          )
        );
      }
    }
  ...
  for (const member of componentMembers) {
    for (const ref of member.outgoingRefs) {
      ...
      refs.push(
        makeOutgoingRefItem(
          targetEntity,
          `${ENTITY_TYPE_LABELS[targetEntity.entityType] || "Сущность"} ${targetEntity.id}`
        )
      );
    }
  }
  ...
  for (const member of componentMembers) {
    ...
    if (aggregate?.entity?.activePublishedRevisionId) {
      stateBlockers.push({
        ...
        label: member.label,
        href: member.href,
        reason: `Объект «${member.label}» сейчас опубликован. Сначала снимите его с публикации.`
      });
    }
    ...
    for (const revision of aggregate?.revisions ?? []) {
      ...
      stateBlockers.push({
        ...
        reason: `Версия объекта «${member.label}» находится на проверке.`
      });
    }
    ...
    for (const obligation of openObligations) {
      stateBlockers.push({
        ...
        label: member.label,
        href: member.href,
        reason: `Для объекта «${member.label}» не завершено действие публикации.`
      });
    }
  }
...
function buildComponentSummary(componentMembers, verdict, publishedIncomingRefs, draftIncomingRefs, stateBlockers) {
  if (verdict === "ready") {
    return "Все проверки пройдены. Эту группу можно безопасно удалить.";
  }

  if (publishedIncomingRefs.length > 0) {
    return "Удаление пока невозможно: объекты используются в опубликованных материалах.";
  }

  if (draftIncomingRefs.length > 0) {
    return "Удаление пока невозможно: объекты используются в рабочих черновиках.";
  }

  if (stateBlockers.length > 0) {
    return "Удаление пока невозможно: сначала завершите указанные действия с объектами.";
  }

  return `Группа из ${componentMembers.length} объектов пока не готова к удалению.`;
}

function buildComponentMembers(componentMembers) {
  return componentMembers.map((member) => {
    const previewUrls = member.entityType === ENTITY_TYPES.MEDIA_ASSET
      ? getAdminMediaPreviewUrls({
        entityId: member.entityId,
        storageKey: member.payload?.storageKey,
        latestRevision: member.latestRevision,
        publishedRevision: member.aggregate?.activePublishedRevision
      })
      : { thumbnailUrl: "" };
...
async function buildComponentModel(componentMembers, latestByType, publishedByType, markedNodeMap, deps, db) {
  ...
  const members = buildComponentMembers(componentMembers);
  const rootMember = members.find((member) => member.entityId === root.entityId) ?? members[0];
  ...
  return {
    root: {
      ...
      label: root.label,
      href: root.href,
      thumbnailUrl: rootMember?.thumbnailUrl || ""
    },
    members,
...
export async function evaluateRemovalSweepComponent(input, deps = {}) {
  ...
  if (!isRemovalQuarantineEntityTypeSupported(entityType)) {
    return {
      exists: false,
      verdict: "blocked",
      summary: "Этот тип пока не поддерживается новым cleanup-контуром."
    };
  }
  ...
  if (!component) {
    return {
      exists: false,
      verdict: "blocked",
      summary: "Помеченный компонент не найден."
    };
  }
