...
const COMPONENT_ROOT_PRIORITY = Object.freeze({
  [ENTITY_TYPES.SERVICE]: 0,
  [ENTITY_TYPES.CASE]: 1,
  [ENTITY_TYPES.GALLERY]: 2,
      ...
      for (const member of componentMembers) {
        ...
        publishedIncomingRefs.push(
          makeIncomingRefItem(
            ...
            publishedCard,
            "published",
            `Опубликованная ${ENTITY_TYPE_LABELS[sourceType] || "сущность"} всё ещё использует этот граф.`
          )
        );
      }
    }
      ...
      for (const member of componentMembers) {
        ...
        draftIncomingRefs.push(
          makeIncomingRefItem(
            ...
            latestCard,
            "draft",
            `Рабочий черновик ${ENTITY_TYPE_LABELS[sourceType] || "сущности"} всё ещё ссылается на этот граф.`
          )
        );
      }
    }
  ...
  for (const member of componentMembers) {
    ...
    if (aggregate?.entity?.activePublishedRevisionId) {
      stateBlockers.push({
        ...
        label: member.label,
        href: member.href,
        reason: `${member.label} всё ещё удерживается живым контуром. В v0.1 опубликованные объекты не purged автоматически.`
      });
    }
    ...
    for (const revision of aggregate?.revisions ?? []) {
      ...
      stateBlockers.push({
        ...
        reason: `${member.label} имеет ревизию на проверке.`
      });
    }
    ...
    for (const obligation of openObligations) {
      stateBlockers.push({
        ...
        label: member.label,
        href: member.href,
        reason: `${member.label} имеет открытое publish-обязательство: ${obligation.obligationType}.`
      });
    }
  }
...
export async function executeRemovalSweep(input, deps = {}) {
  ...
  if (evaluation.verdict !== "ready") {
    await resolvedDeps.recordAuditEvent({
      entityId: rootEntityId,
      actorUserId,
      eventKey: AUDIT_EVENT_KEYS.REMOVAL_SWEEP_BLOCKED,
  ...
  const transactionResult = await resolvedDeps.withTransaction(async (db) => {
    ...
    for (const item of purgePlan) {
      const aggregate = await resolvedDeps.getEntityAggregate(item.entityId, db);

      if (item.entityType === ENTITY_TYPES.MEDIA_ASSET && aggregate) {
        storageKeys.push(...collectMediaStorageKeys(aggregate));
      }
  ...
  await resolvedDeps.recordAuditEvent({
    entityId: null,
    actorUserId,
    eventKey: AUDIT_EVENT_KEYS.REMOVAL_SWEEP_EXECUTED,
