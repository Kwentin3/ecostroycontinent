...
const CATEGORY_LABELS = Object.freeze({
  ...
  "contact truth gap": "Пробел в контактах",
  unknown: "Неизвестно"
});
...
const STATE_NOTES = Object.freeze({
  blocked: "Ниже видны блокирующие точки с доказательствами.",
  partial: "Доказательства есть, но часть точек ещё открыта.",
  ready: "Пробелов в доказательствах не видно.",
  missing: "Доказательства ещё не собраны полностью."
});
...
function getFieldLabel(field) {
  return FIELD_LABELS[field] || field || "раздел";
}
...
function getScopeLabel(scope) {
  return scope === "editor" ? "Текущая сущность" : "Рабочий набор";
}

function getTargetLabel({ scope, anchor, fieldLabel }) {
  if (anchor.isFallback) {
    return "Открыть резервный раздел";
  }

  return scope === "editor" ? `Перейти к ${fieldLabel}` : `Открыть ${fieldLabel}`;
}
...
function buildTarget({ scope, entityType, entityId, anchor, fieldLabel }) {
  ...
  return {
    ...
    targetLabel: anchor.isFallback ? "Резервный раздел" : fieldLabel,
...
function projectEvidenceItems({
  ...
  return items.map((item, index) => {
    ...
    return {
      ...
      fieldLabel,
      reason: normalizeText(item.reason) || "Причина не указана.",
