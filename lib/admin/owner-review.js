...
// Review-gallery status is about approval flow, not live publication.
// `state=review` is only a technical lane; the owner-decision queue is the
// narrower `ownerReviewRequired + ownerApprovalStatus=pending` state.
// "Approved" means the candidate revision got the needed review decision and
// can now move to explicit publish from the entity screen.
export function getOwnerReviewStatusModel(revision) {
  ...
  if (status.key === "needs_owner") {
    return {
      ...status,
      description: "Материал ждёт решения собственника."
    };
  }
  ...
  if (status.key === "returned") {
    return {
      ...status,
      description: revision.reviewComment ? excerpt(revision.reviewComment, 120) : "Материал возвращён SEO на доработку."
    };
  }
  ...
  return {
    ...status,
    description: "Материал находится во внутренней проверке без отдельного решения собственника."
  };
}
...
function buildGlobalSettingsCard(item) {
  const { payload } = item.revision;

  return {
    title: payload.publicBrandName || payload.legalName || getEntityTypeLabel(item.entityType),
    summary: pickFirstExcerpt(
      [
        payload.primaryPhone ? `Телефон: ${payload.primaryPhone}` : "",
        payload.publicEmail ? `Почта: ${payload.publicEmail}` : "",
        payload.serviceArea ? `Зона: ${payload.serviceArea}` : "",
        payload.contactTruthConfirmed ? "Контактные данные подтверждены" : "Контактные данные не подтверждены"
      ],
      "Проверьте контактные данные, зону обслуживания и базовый призыв к обращению."
    ),
    facts: uniqueCompactFacts([
...
function buildGalleryCard(item) {
  ...
  return {
    title: payload.title || getEntityTypeLabel(item.entityType),
    summary: pickFirstExcerpt(
      [payload.caption, payload.assetIds?.length ? `Файлов: ${payload.assetIds.length}` : ""],
      "Проверьте состав коллекции, главный кадр и подпись."
    ),
    facts: uniqueCompactFacts([
      payload.caption,
      payload.primaryAssetId ? "Главный файл выбран" : "",
      payload.assetIds?.length ? `Файлов: ${payload.assetIds.length}` : "",
      payload.relatedEntityIds?.length ? `Связей: ${payload.relatedEntityIds.length}` : ""
    ]),
    mediaUrl: getPageCardPreviewUrl(payload.primaryAssetId)
  };
}
...
function buildServiceModalModel(item) {
  ...
  return {
    title: card.title,
    summary: fullText(payload.summary) || card.summary,
    mediaUrl: card.mediaUrl,
    sections: compactSections([
      { label: "Что входит", value: fullText(payload.serviceScope) },
      { label: "Какую задачу решает", value: fullText(payload.problemsSolved) },
      { label: "Как выполняем", value: fullText(payload.methods) }
    ]),
...
function buildCaseModalModel(item) {
  ...
  return {
    title: card.title,
    summary: fullText(payload.summary) || fullText(payload.result) || card.summary,
    mediaUrl: card.mediaUrl,
    sections: compactSections([
      { label: "Где и на каком объекте", value: fullText([payload.location, payload.projectType]) },
      { label: "Что делали", value: fullText([payload.task, payload.workScope]) },
      { label: "Какой результат", value: fullText(payload.result) }
    ]),
...
function buildMediaModalModel(item) {
  ...
  return {
    title: card.title,
    summary: fullText(payload.caption) || fullText(payload.alt) || card.summary,
    mediaUrl: card.mediaUrl,
    sections: compactSections([
      { label: "Подпись", value: fullText(payload.caption) },
      { label: "Описание фото", value: fullText(payload.alt) },
      { label: "Примечание", value: fullText([payload.ownershipNote, payload.sourceNote]) }
    ]),
...
function buildEquipmentModalModel(item) {
  ...
  return {
    title: card.title,
    summary: fullText(payload.shortSummary) || card.summary,
    mediaUrl: card.mediaUrl,
    sections: compactSections([
      { label: "Тип техники", value: fullText(payload.equipmentType) },
      { label: "Для каких задач", value: fullText(payload.capabilitySummary) },
      { label: "Ключевые характеристики", value: fullText(payload.keySpecs) },
      { label: "Сценарии применения", value: fullText(payload.usageScenarios) }
    ]),
    commentPlaceholder: "Например: техника выбрана верно, но нужно уточнить описание возможностей или заменить фото."
  };
}

function buildGlobalSettingsModalModel(item) {
  const { payload } = item.revision;
  const card = buildGlobalSettingsCard(item);
  ...
  return {
    ...
    mediaUrl: card.mediaUrl,
    sections: compactSections([
      { label: "Телефон", value: fullText(payload.primaryPhone) },
      { label: "Почта", value: fullText(payload.publicEmail) },
      { label: "Мессенджеры", value: fullText(payload.activeMessengers) },
      { label: "Зона обслуживания", value: fullText([payload.serviceArea, payload.primaryRegion]) },
      { label: "Призыв к обращению", value: fullText([payload.defaultCtaLabel, payload.defaultCtaDescription]) },
      { label: "Подтверждение контактов", value: payload.contactTruthConfirmed ? "Подтверждены" : "Не подтверждены" }
    ]),
    commentPlaceholder: "Например: телефон верный, но нужно уточнить мессенджеры или зону обслуживания."
  };
}

function buildGalleryModalModel(item) {
  const { payload } = item.revision;
  const card = buildGalleryCard(item);

  return {
    title: card.title,
    summary: fullText(payload.caption) || card.summary,
    mediaUrl: card.mediaUrl,
    sections: compactSections([
      { label: "Подпись", value: fullText(payload.caption) },
      { label: "Состав", value: payload.assetIds?.length ? `В коллекции ${payload.assetIds.length} файлов.` : "" },
      { label: "Главный файл", value: fullText(payload.primaryAssetId) },
      { label: "Связанные материалы", value: payload.relatedEntityIds?.length ? `Связей: ${payload.relatedEntityIds.length}` : "" }
    ]),
    commentPlaceholder: "Например: коллекция подходит, но нужно заменить главный кадр или подпись."
  };
}
...
function buildPageModalModel(item) {
  ...
  return {
    title: card.title,
    summary: fullText(pageValue.intro) || card.summary,
    mediaUrl: card.mediaUrl,
    pageValue,
    sections: compactSections([
      { label: "Тип страницы", value: PAGE_TYPE_LABELS[pageValue.pageType] || "" },
      { label: "Вводный блок", value: fullText(pageValue.intro) },
...
function buildCardPayload(item) {
  switch (item.entityType) {
    case ENTITY_TYPES.GLOBAL_SETTINGS:
      return buildGlobalSettingsCard(item);
...
export function buildOwnerReviewModalModel(item) {
  switch (item.entityType) {
    case ENTITY_TYPES.GLOBAL_SETTINGS:
      return buildGlobalSettingsModalModel(item);
    case ENTITY_TYPES.SERVICE:
    ...
    case ENTITY_TYPES.GALLERY:
      return buildGalleryModalModel(item);
    case ENTITY_TYPES.PAGE:
