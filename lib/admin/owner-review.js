...
export function getOwnerReviewStatusModel(revision) {
  ...
  if (revision.ownerApprovalStatus === "approved") {
    return {
      key: "in_review",
      label: "На проверке",
      secondaryLabel: "Согласовано",
      description: "Согласование получено. Материал остается в общей проверке: следующий шаг — открыть публикацию.",
      attention: false
    };
  }
...
export function filterOwnerReviewGalleryCards(cards = [], filters = {}) {
  ...
  const rawStatus = typeof filters.status === "string" ? filters.status : "all";
  const status = rawStatus === "approved" ? "in_review" : rawStatus;
