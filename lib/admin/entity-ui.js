...
import { findEntityById, findEntityByTypeSingleton, findRevisionById, listPublishObligations } from "../content-core/repository.js";
import {
  listLegacyEquipmentLinkedToEntity,
  resolveEquipmentIdsForEntity
} from "../content-core/equipment-relations.js";
...
export async function loadEditorPageData(entityType, entityId) {
  ...
  const visibleServices = services.filter((card) => !card.entity?.markedForRemovalAt);
  const visibleEquipment = equipment.filter((card) => !card.entity?.markedForRemovalAt);
  const visibleCases = cases.filter((card) => !card.entity?.markedForRemovalAt);
  const reverseEquipment = entityId && [ENTITY_TYPES.SERVICE, ENTITY_TYPES.CASE].includes(entityType)
    ? listLegacyEquipmentLinkedToEntity(visibleEquipment, entityType, entityId)
    : [];
  const referencingServices = entityId && entityType === ENTITY_TYPES.EQUIPMENT
    ? visibleServices.filter((card) => resolveEquipmentIdsForEntity({
        payload: card.latestRevision?.payload || {},
        equipmentRecords: visibleEquipment,
        entityType: ENTITY_TYPES.SERVICE,
        entityId: card.entity.id
      }).includes(entityId))
    : [];
  const referencingCases = entityId && entityType === ENTITY_TYPES.EQUIPMENT
    ? visibleCases.filter((card) => resolveEquipmentIdsForEntity({
        payload: card.latestRevision?.payload || {},
        equipmentRecords: visibleEquipment,
  ...
  const visibleGalleries = (
    await Promise.all(
      galleries.map(async (card) => ({
        ...card,
        entity: await findEntityById(card.entityId)
      }))
    )
  ).filter((card) => !card.entity?.markedForRemovalAt);
  ...
  return {
    ...
    obligations,
    relationOptions: {
      services: visibleServices.map(optionFromCard),
      equipment: visibleEquipment.map(optionFromCard),
      reverseEquipment: reverseEquipment.map(optionFromCard),
      referencingServices: referencingServices.map(optionFromCard),
      referencingCases: referencingCases.map(optionFromCard),
      cases: visibleCases.map(optionFromCard),
      galleries: visibleGalleries.map((card) => ({
        id: card.entityId,
    ...
    globalSettings: globalSettingsRevision?.payload || null,
    mediaOptions,
...
const PREVIEW_TARGETS = {
  [ENTITY_TYPES.SERVICE]: {
    ...
    serviceScopeDisplayMode: "preview-service-scope",
    problemsSolved: "preview-service-scope",
    methods: "preview-service-scope",
    equipmentIds: "preview-service-related-equipment",
  ...
  [ENTITY_TYPES.CASE]: {
    ...
    equipmentIds: "preview-case-related-equipment",
