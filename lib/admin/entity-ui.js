...
import { ADMIN_COPY, getEntityTypeLabel, getRevisionStateLabel } from "../ui-copy.js";
...
export async function loadEditorPageData(entityType, entityId) {
  ...
  return {
    ...
    mediaOptions: mediaAssets.map((card) => ({
      id: card.entityId,
      title: card.revision?.payload?.title || card.revision?.payload?.originalFilename || ADMIN_COPY.untitledAsset,
      alt: card.revision?.payload?.alt || "",
      originalFilename: card.revision?.payload?.originalFilename || "",
      whereUsedLabel: "Пока не используется",
