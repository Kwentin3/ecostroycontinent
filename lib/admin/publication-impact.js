import { ENTITY_TYPES, ENTITY_TYPE_LABELS, PAGE_TYPES } from "../content-core/content-types.js";
import {
  getEntityAggregate,
...
export const PUBLICATION_ENTITY_TYPES = Object.freeze([
  ENTITY_TYPES.PAGE,
...
const PUBLICATION_ENTITY_TYPE_SET = new Set(PUBLICATION_ENTITY_TYPES);

export const PUBLICATION_ACTIVE_REVISION_CHANGED_BLOCKER = "Опубликованная версия изменилась во время операции. Обновите страницу и повторите действие.";

export function dedupeStrings(items = []) {
  return [...new Set(items.filter(Boolean))];
}

export function dedupeItems(items = [], keyBuilder) {
  const seen = new Set();
  const result = [];
...
export function isPublicationEntityTypeSupported(entityType) {
  return PUBLICATION_ENTITY_TYPE_SET.has(String(entityType ?? "").trim());
}

export function buildUnsupportedPublicationImpact(entityType, entityId) {
  return {
    entityType,
    entityId,
    exists: false,
...
export function buildNotFoundPublicationImpact(entityType, entityId) {
  return {
    entityType,
    entityId,
    exists: false,
    supported: isPublicationEntityTypeSupported(entityType),
...
export function buildPublicationRouteEffects(entityType, publishedPayload = {}, entityId = "") {
  if (entityType === ENTITY_TYPES.PAGE) {
    ...
    if (pageType === PAGE_TYPES.HOME) {
      return {
        allowed: true,
        routePath: "/",
        routeOutcome: "Маршрут / станет 404, пока не появится новая опубликованная главная страница.",
        listImpact: "Отдельного листинга страниц нет.",
        sitemapImpact: "Если sitemap строится из опубликованного контура, запись / должна исчезнуть.",
        revalidationPaths: ["/"]
      };
    }
...
export async function collectPublicationImpact(input, deps = {}) {
  const resolvedDeps = {
    getEntityAggregate,
    listPublishObligations,
    listEntityCards,
  ...
  if (!isPublicationEntityTypeSupported(entityType)) {
    return buildUnsupportedPublicationImpact(entityType, entityId);
  }
  ...
  if (!aggregate?.entity || aggregate.entity.entityType !== entityType) {
    return buildNotFoundPublicationImpact(entityType, entityId);
  }
  ...
  const reviewResidue = (aggregate.revisions ?? [])
    .filter((revision) => revision.state === "review")
    .map(buildReviewResidueItem);
  const openObligationItems = openObligations
    .map((obligation) => buildOpenObligationItem(entityType, entityId, obligation));
  const routeEffects = buildPublicationRouteEffects(
    entityType,
    publishedRevision?.payload ?? latestRevision?.payload ?? {},
    entityId
  );
  const incoming = await collectIncomingReferences(entityType, entityId, resolvedDeps, db);
  const warnings = [];

  if (routeEffects.allowed && routeEffects.routeOutcome) {
  ...
  return {
    ...
    incomingBlockers: incoming.blockers,
    ...
    unsupportedRouteEffectReason: routeEffects.allowed ? "" : routeEffects.reason,
    root: {
...
export function buildPublicationForensicDetails(evaluation) {
  return {
