...
export async function evaluateLegacyTestFixtureNormalization(input, deps = {}) {
  ...
  if (!isLegacyTestFixtureNormalizationEntityTypeSupported(entityType)) {
    return {
      ...
      exists: false,
      allowed: false,
      blockers: ["Этот тип сущности не поддерживает нормализацию legacy test fixture."],
      warnings: [],
      publishedIncomingRefs: [],
      draftIncomingRefs: [],
      relatedTargets: [],
  ...
  if (aggregate.entity.creationOrigin && !isAgentTestCreationOrigin(aggregate.entity.creationOrigin)) {
    blockers.push("Сущность уже имеет другой persisted marker и не может быть нормализована этим bridge-срезом.");
  }
  ...
  if (aggregate.entity.activePublishedRevisionId) {
    warnings.push("После пометки объект перестанет считаться ordinary live entity и должен будет разбираться через удаление тестового графа.");
  }

  if ((aggregate.revisions ?? []).some((revision) => revision.state === "review")) {
    warnings.push("Review/publish residue сохранится: teardown всё ещё будет честно заблокирован, пока объект участвует в review/publish-потоке.");
  }
...
export async function executeLegacyTestFixtureNormalization(input, deps = {}) {
  ...
  return resolvedDeps.withTransaction(async (db) => {
    ...
    await resolvedDeps.recordAuditEvent({
      ...
      summary: "Legacy test fixture помечен как test-marked объект для teardown-пути.",
      details: {
