...
const eventPayloadSchema = z.strictObject({
  ...
  anonymous_id: z.string().trim().min(8).max(128).optional(),
...
function createServerGeneratedId(prefix) {
...
export function validateAnalyticsEventPayload(input, { request = null, user = null, now = new Date() } = {}) {
  ...
  try {
    ...
    const event = {
      ...
      anonymous_id: payload.anonymous_id || createServerGeneratedId("anon_server"),
      session_id: payload.session_id || createServerGeneratedId("session_server"),
      page_path: pagePath,
