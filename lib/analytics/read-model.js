...
// Boundary: this is a consumer DTO for UI/LLM/reports, not source of truth.
// UI/LLM must not reassemble metrics from raw events, Content Core or Yandex.
// Contact intent aggregates are not lead records; lead/intake is separate.
// See Analytics Read Model Contract and docs/handbook/SEO_DASHBOARD_CURRENT_STATE_AND_AGENT_HANDOFF_Экостройконтинент_v0.1.md.
const DAY_MS = 24 * 60 * 60 * 1000;
const METRICA_R2B_REPORT_TYPES = new Set(["traffic_source", "source_detail", "device", "country", "region", "landing_url"]);
...
function ageHours(value, now) {
  ...
  return (new Date(now).getTime() - parsed.getTime()) / (60 * 60 * 1000);
}
...
function dateAgeDays(value, now) {
  ...
  const current = sourceDate(now);
  const [currentYear, currentMonth, currentDay] = current.split("-").map((part) => Number(part));

  return Math.floor((Date.UTC(currentYear, currentMonth - 1, currentDay) - Date.UTC(year, month - 1, day)) / DAY_MS);
}

function sourceFreshness(sourceId, sourceState, now) {
  const status = sourceState?.status || "not_configured";
  const threshold = sourceId === "yandex_metrica"
    ? "last_successful_at <= 48h and imported_period_end <= 3 completed days"
    : "last_successful_at <= 7 days";
  ...
  const lastSuccessAgeHours = ageHours(sourceState.last_successful_at, now);

  if (sourceId === "yandex_metrica") {
    const importedPeriodAgeDays = dateAgeDays(sourceState.imported_period_end, now);
    const fresh = lastSuccessAgeHours <= 48 && importedPeriodAgeDays <= 3;

    return {
      status: fresh ? "fresh" : "stale",
      threshold,
      reason: fresh ? "within_threshold" : "last_success_or_imported_period_too_old"
    };
  }

  const fresh = lastSuccessAgeHours <= 24 * 7;

  return {
    status: fresh ? "fresh" : "stale",
    threshold,
    reason: fresh ? "within_threshold" : "last_success_too_old"
  };
}

function normalizeSourceStateForReadiness(sourceState) {
  return {
    status: sourceState.status,
    last_attempted_at: sourceDateTime(sourceState.last_attempted_at),
    last_successful_at: sourceDateTime(sourceState.last_successful_at),
    imported_period_start: sourceDate(sourceState.imported_period_start),
    imported_period_end: sourceDate(sourceState.imported_period_end),
    rows_imported: number(sourceState.rows_imported),
    safe_error_message: sourceState.safe_error_message || ""
  };
}

function buildMetricaReadiness({ sourceState, summary, now }) {
  const importedSummary = {
    traffic_rows: number(summary?.traffic_rows),
    goal_rows: number(summary?.goal_rows),
    r2b_rows: number(summary?.r2b_rows),
    nonzero_rows: number(summary?.nonzero_rows),
    all_values_zero: Boolean(summary?.all_values_zero),
    report_types: Array.isArray(summary?.report_types) ? summary.report_types : []
  };
  const limitations = [
    "external_metrica_not_operational_truth",
    "metrica_external_enrichment_only"
  ];
  const hasR2bReports = importedSummary.report_types.some((reportType) => METRICA_R2B_REPORT_TYPES.has(reportType));

  if (!hasR2bReports) {
    limitations.push("metrica_dimensions_r2b_not_imported");
  }

  if (!number(summary?.total_rows)) {
    limitations.push("external_metrica_no_imported_rows");
  }

  if (importedSummary.all_values_zero) {
    limitations.push("external_metrica_all_values_zero");
  }

  if (sourceState.status !== "ok") {
    limitations.push(`external_metrica_source_${sourceState.status}`);
  }

  return {
    ...normalizeSourceStateForReadiness(sourceState),
    freshness: sourceFreshness("yandex_metrica", sourceState, now),
    data_actionability: hasR2bReports && importedSummary.nonzero_rows > 0 ? "limited_external_evidence" : importedSummary.nonzero_rows > 0 ? "limited_external_diagnostic" : "readiness_only",
    limitations,
...
function safeCountObject(value) {
  ...
  return Object.fromEntries(Object.entries(value).map(([key, nested]) => [key, number(nested)]));
}

function buildWebmasterReadiness({
  sourceState,
  hostSnapshot,
  indexationSnapshot,
  urlSampleSummary,
  ...
  const metrics = indexationSnapshot?.metrics && typeof indexationSnapshot.metrics === "object"
    ? indexationSnapshot.metrics
    : {};
  const queryRows = number(queryVisibilitySummary?.query_visibility_rows);
  const importedSummary = {
    host_verified: Boolean(hostSnapshot?.verified),
    host_data_status: hostSnapshot?.host_data_status || "",
    searchable_pages_count: number(metrics.searchable_pages_count),
    excluded_pages_count: number(metrics.excluded_pages_count),
    site_problem_counts: safeCountObject(metrics.site_problems),
    url_sample_count: number(urlSampleSummary?.url_sample_count),
    resolved_url_sample_count: number(urlSampleSummary?.resolved_url_sample_count),
    query_visibility_rows: queryRows
  };
  const limitations = [
  ...
  if (queryRows === 0) {
    limitations.push("webmaster_query_visibility_no_rows_for_period");
  }

  if (sourceState.status !== "ok") {
    limitations.push(`webmaster_source_${sourceState.status}`);
  }

  return {
    ...normalizeSourceStateForReadiness(sourceState),
    unmapped_url_count: number(sourceState.unmapped_url_count),
    freshness: sourceFreshness("yandex_webmaster", sourceState, now),
    data_actionability: queryRows > 0 ? "limited_search_visibility_evidence" : "readiness_and_limited_indexation_evidence",
    limitations,
    imported_summary: importedSummary
  };
}

function buildExternalSourceReadiness({
  sourceStates,
  ...
  return {
    yandex_metrica: buildMetricaReadiness({
      sourceState: sourceStates.yandex_metrica,
      summary: metricaImportSummary,
      now
    }),
    yandex_webmaster: buildWebmasterReadiness({
      sourceState: sourceStates.yandex_webmaster,
      hostSnapshot: webmasterHostSnapshot,
      indexationSnapshot: webmasterIndexationSnapshot,
      urlSampleSummary: webmasterUrlSampleSummary,
      queryVisibilitySummary: webmasterQueryVisibilitySummary,
      now
    })
  };
}

function metricTotals(value = {}) {
  return {
    row_count: number(value.row_count),
    visits: number(value.visits),
    users: number(value.users),
    pageviews: number(value.pageviews),
    period_start: sourceDate(value.period_start),
    period_end: sourceDate(value.period_end)
  };
}

function metricRow(row, fields) {
  const result = {};
  ...
  return {
    ...
    period_start: sourceDate(row?.period_start),
    period_end: sourceDate(row?.period_end)
  };
}

function compactMetricRows(rows = [], fields) {
  return rows.map((row) => metricRow(row, fields));
}

function hasMetricData(totals = {}) {
  return number(totals.visits) + number(totals.users) + number(totals.pageviews) > 0;
}

function buildMetricaExternalEvidence({
  readiness,
  summary,
  ...
  const reportTypes = Array.isArray(summary?.report_types) ? summary.report_types : [];
  const hasR2bReports = reportTypes.some((reportType) => METRICA_R2B_REPORT_TYPES.has(reportType));
  const trafficTotals = metricTotals(trafficSourceEvidence?.totals);
  const limitations = [
    "external_metrica_not_operational_truth",
    "metrica_external_enrichment_only",
  ...
  if (!hasR2bReports) {
    limitations.push("external_metrica_no_r2b_rows");
  }

  if (hasR2bReports && trafficTotals.visits > 0 && trafficTotals.visits < 30) {
    limitations.push("low_external_sample_size");
  }

  if (readiness.status !== "ok") {
    limitations.push(`external_metrica_source_${readiness.status}`);
  }

  const sourceDetailTotals = metricTotals(sourceDetailEvidence?.totals);
  const countryTotals = metricTotals(geographyEvidence?.country_totals);
  const regionTotals = metricTotals(geographyEvidence?.region_totals);
  const landingTotals = metricTotals(landingEvidence?.totals);

  return {
    status: readiness.status,
    freshness: readiness.freshness,
    data_actionability: hasR2bReports && hasMetricData(trafficTotals) ? "limited_external_evidence" : "readiness_only",
    limitations,
    imported_period_start: readiness.imported_period_start,
    imported_period_end: readiness.imported_period_end,
    traffic_sources: {
      rows: compactMetricRows(trafficSourceEvidence?.rows || [], ["traffic_source", "traffic_source_name"]),
      totals: trafficTotals,
      limitations: ["attribution_model_lastsign", "external_metrica_not_operational_truth"]
    },
    source_details: {
      rows: compactMetricRows(sourceDetailEvidence?.rows || [], ["traffic_source", "traffic_source_name", "source_engine", "source_engine_name"]),
      totals: sourceDetailTotals,
      limitations: reportTypes.includes("source_detail") ? ["optional_external_detail"] : ["metrica_source_detail_not_available_or_skipped"]
    },
    devices: {
      rows: compactMetricRows(deviceEvidence?.rows || [], ["device_category", "device_category_name"]),
      totals: metricTotals(deviceEvidence?.totals),
      limitations: ["external_metrica_not_operational_truth"]
    },
    geography: {
      countries: compactMetricRows(geographyEvidence?.countries || [], ["country", "country_name"]),
      regions: compactMetricRows(geographyEvidence?.regions || [], ["country", "country_name", "region_area", "region_area_name"]),
      country_totals: countryTotals,
      region_totals: regionTotals,
      limitations: reportTypes.includes("region") ? ["country_and_region_are_external_aggregates"] : ["metrica_region_not_available_or_skipped"]
    },
    landings: {
      rows: compactMetricRows(landingEvidence?.rows || [], ["page_path", "normalized_url", "entity_type", "entity_id"]),
      totals: landingTotals,
      mapped_count: number(landingEvidence?.mapped_count),
      unmapped_count: number(landingEvidence?.unmapped_count),
      limitations: ["metrica_landing_mapping_read_only", "unmapped_urls_are_diagnostics_only"]
    }
  };
}

function buildWebmasterExternalEvidence({
  readiness,
  hostSnapshot,
  indexationSnapshot,
  urlSampleSummary,
  ...
  if (readiness.status !== "ok") {
    limitations.push(`webmaster_source_${readiness.status}`);
  }
  ...
  return {
    ...
    url_samples: {
      sample_count: number(urlSampleSummary?.url_sample_count),
      resolved_count: number(urlSampleSummary?.resolved_url_sample_count),
      unmapped_count: Math.max(0, number(urlSampleSummary?.url_sample_count) - number(urlSampleSummary?.resolved_url_sample_count)),
      rows: (urlSampleEvidence || []).map((row) => ({
        ...
        observed_date: sourceDate(row.observed_date),
        sample_status: row.sample_status || "",
        http_code: row.http_code === null || row.http_code === undefined ? null : number(row.http_code),
        title: row.title || ""
      })),
      limitations: ["webmaster_url_samples_are_not_full_coverage"]
    },
    query_visibility: {
      row_count: queryRows,
      rows: (queryVisibilityEvidence || []).map((row) => ({
        ...
        ctr: number(row.ctr),
        average_position: row.average_position === null || row.average_position === undefined ? null : number(row.average_position)
      })),
      totals: {
        impressions: number(queryVisibilitySummary?.impressions),
        clicks: number(queryVisibilitySummary?.clicks),
        min_date: sourceDate(queryVisibilitySummary?.min_date),
        max_date: sourceDate(queryVisibilitySummary?.max_date)
      },
      limitations: queryRows === 0 ? ["webmaster_query_visibility_no_rows_for_period", "no_zero_demand_claim"] : ["external_search_visibility_evidence_only"]
    }
  };
}
...
function buildExternalEvidence({
  ...
  // External evidence is enrichment only: no primary overview, no recommendations, no Content Core mutation.
  return {
    yandex_metrica: buildMetricaExternalEvidence({
      readiness: externalSourceReadiness.yandex_metrica,
      summary: metricaImportSummary,
      trafficSourceEvidence: metricaTrafficSourceEvidence,
      sourceDetailEvidence: metricaSourceDetailEvidence,
      deviceEvidence: metricaDeviceEvidence,
      geographyEvidence: metricaGeographyEvidence,
      landingEvidence: metricaLandingEvidence
    }),
    yandex_webmaster: buildWebmasterExternalEvidence({
      readiness: externalSourceReadiness.yandex_webmaster,
      hostSnapshot: webmasterHostSnapshot,
      indexationSnapshot: webmasterIndexationSnapshot,
      urlSampleSummary: webmasterUrlSampleSummary,
      urlSampleEvidence: webmasterUrlSampleEvidence,
      queryVisibilitySummary: webmasterQueryVisibilitySummary,
      queryVisibilityEvidence: webmasterQueryVisibilityEvidence
    })
  };
}
...
function pagePathForRecord(record) {
  ...
  if ((record.entityType || record.entity_type) === ENTITY_TYPES.PAGE) {
    if (payload.pageType === PAGE_TYPES.HOME) {
      return "/";
    }
...
export async function buildSeoDashboardReadModel({
  ...
  const sourceStateRows = await (hasDep("sourceStates") ? deps.sourceStates : listSourceSyncStates(db));
  ...
  const [
    ...
    hasDep("currentRows") ? deps.currentRows : listAnalyticsPageDaily({ startDate: periods.current.start, endDate: periods.current.end }, db),
    hasDep("previousRows") ? deps.previousRows : listAnalyticsPageDaily({ startDate: periods.previous.start, endDate: periods.previous.end }, db),
    hasDep("searchRows") ? deps.searchRows : listExternalSearchVisibility({ startDate: periods.current.start, endDate: periods.current.end }, db),
    hasDep("previousSearchRows") ? deps.previousSearchRows : listExternalSearchVisibility({ startDate: periods.previous.start, endDate: periods.previous.end }, db),
    hasDep("unmappedDiagnostics") ? deps.unmappedDiagnostics : listUnmappedUrlDiagnostics({ limit: 20 }, db),
    hasDep("recommendationStates") ? deps.recommendationStates : listRecommendationStates(db),
    hasDep("trackingChanges") ? deps.trackingChanges : listTrackingChangeHistory({ limit: 30 }, db),
    hasDep("metricaImportSummary") ? deps.metricaImportSummary : getMetricaImportSummary({
      startDate: sourceStates.yandex_metrica.imported_period_start,
      endDate: sourceStates.yandex_metrica.imported_period_end
    }, db),
    hasDep("webmasterHostSnapshot") ? deps.webmasterHostSnapshot : getLatestWebmasterHostSnapshot(db),
    hasDep("webmasterIndexationSnapshot") ? deps.webmasterIndexationSnapshot : getLatestWebmasterIndexationSnapshot(db),
    hasDep("webmasterUrlSampleSummary") ? deps.webmasterUrlSampleSummary : getWebmasterUrlSampleSummary(db),
    hasDep("webmasterQueryVisibilitySummary") ? deps.webmasterQueryVisibilitySummary : getWebmasterQueryVisibilitySummary({
      startDate: sourceStates.yandex_webmaster.imported_period_start,
      endDate: sourceStates.yandex_webmaster.imported_period_end
    }, db),
    hasDep("metricaTrafficSourceEvidence") ? deps.metricaTrafficSourceEvidence : getMetricaTrafficSourceEvidence({
      startDate: sourceStates.yandex_metrica.imported_period_start,
      endDate: sourceStates.yandex_metrica.imported_period_end,
      limit: 10
    }, db),
    hasDep("metricaSourceDetailEvidence") ? deps.metricaSourceDetailEvidence : getMetricaSourceDetailEvidence({
      startDate: sourceStates.yandex_metrica.imported_period_start,
      endDate: sourceStates.yandex_metrica.imported_period_end,
      limit: 10
    }, db),
    hasDep("metricaDeviceEvidence") ? deps.metricaDeviceEvidence : getMetricaDeviceEvidence({
      startDate: sourceStates.yandex_metrica.imported_period_start,
      endDate: sourceStates.yandex_metrica.imported_period_end,
      limit: 10
    }, db),
    hasDep("metricaGeographyEvidence") ? deps.metricaGeographyEvidence : getMetricaGeographyEvidence({
      startDate: sourceStates.yandex_metrica.imported_period_start,
      endDate: sourceStates.yandex_metrica.imported_period_end,
      limit: 10
    }, db),
    hasDep("metricaLandingEvidence") ? deps.metricaLandingEvidence : getMetricaLandingEvidence({
      startDate: sourceStates.yandex_metrica.imported_period_start,
      endDate: sourceStates.yandex_metrica.imported_period_end,
      limit: 10
    }, db),
    hasDep("webmasterUrlSampleEvidence") ? deps.webmasterUrlSampleEvidence : getWebmasterUrlSampleEvidence({ limit: 10 }, db),
    hasDep("webmasterQueryVisibilityEvidence") ? deps.webmasterQueryVisibilityEvidence : getWebmasterQueryVisibilityEvidence({
      startDate: sourceStates.yandex_webmaster.imported_period_start,
      endDate: sourceStates.yandex_webmaster.imported_period_end,
      limit: 10
    }, db)
  ]);
  const externalSourceReadiness = buildExternalSourceReadiness({
    sourceStates,
    metricaImportSummary,
    webmasterHostSnapshot,
    webmasterIndexationSnapshot,
  ...
  const recommendationDiagnostics = unmappedDiagnostics.filter((diagnostic) => !diagnostic.source_system || diagnostic.source_system === "first_party_events");

  for (const diagnostic of recommendationDiagnostics) {
    const page = {
  ...
  return {
    ...
    external_source_readiness: externalSourceReadiness,
