...
// Working edits branch from the current draft or the active published revision.
// They must not mutate live truth in place; see CONTENT_DOMAIN_INTERACTION_CONTRACT_v1.

function isRevisionNumberNotAfter(left, right) {
  const leftNumber = Number(left?.revisionNumber);
  const rightNumber = Number(right?.revisionNumber);
...
export function getEditableDraftRevision(aggregate) {
  const draft = aggregate?.revisions?.find((revision) => revision.state === "draft") ?? null;
  const activePublishedRevision = aggregate?.activePublishedRevision ?? null;

  if (!draft || !activePublishedRevision) {
    return draft;
  }

  return isRevisionNumberNotAfter(draft, activePublishedRevision) ? null : draft;
}

export function getWorkingRevision(aggregate) {
  return getEditableDraftRevision(aggregate) ?? aggregate?.activePublishedRevision ?? null;
}

export function resolveDraftOwnerApprovalStatus({ currentDraft = null, ownerReviewRequired = false } = {}) {
  if (!ownerReviewRequired) {
    return "not_required";
  }

  return currentDraft?.ownerApprovalStatus === "rejected" ? "rejected" : "pending";
}
...
export async function saveDraft(input, options = {}) {
  ...
  const contentFingerprint = computeContentFingerprint(nextPayload);
  ...
  if (currentDraft) {
    revision = await updateRevision(currentDraft.id, {
      ...
      ownerReviewRequired,
      ownerApprovalStatus: resolveDraftOwnerApprovalStatus({ currentDraft, ownerReviewRequired }),
    ...
    revision = await createRevision({
      ...
      changeIntent: input.changeIntent,
      ownerReviewRequired,
      ownerApprovalStatus: resolveDraftOwnerApprovalStatus({ ownerReviewRequired }),
