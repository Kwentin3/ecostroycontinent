...
function mapEntityRow(row) {
  ...
  return {
    ...
    markedForRemovalAt: row.marked_for_removal_at ?? null,
    markedForRemovalBy: row.marked_for_removal_by ?? null,
...
function mapAuditEventRow(row) {
  ...
  return {
    ...
    summary: row.summary,
    details: row.details,
    revisionPayload: row.revision_payload ?? null,
    createdAt: row.created_at
  };
}

function mapDestructiveForensicEventRow(row) {
  if (!row) {
    return null;
  }
  ...
  return {
    ...
    rootEntityLabel: row.root_entity_label,
    targetEntityId: row.target_entity_id,
    targetEntityType: row.target_entity_type,
    targetEntityLabel: row.target_entity_label,
...
export async function createEntity(entityType, userId, db = null, options = {}) {
  const entityId = createId("entity");
  const result = await queryWithDb(
    db,
    `INSERT INTO content_entities (id, entity_type, creation_origin, created_by, updated_by)
     VALUES ($1, $2, $3, $4, $4)
     RETURNING *`,
    [entityId, entityType, options.creationOrigin ?? null, userId]
  );
...
export async function createRevision(input, db = null) {
  ...
  const result = await queryWithDb(
    ...
    `INSERT INTO content_revisions (
      ...
      content_fingerprint, superseded_by_revision_id,
    ...
    [
      ...
      input.supersededByRevisionId ?? null,
...
export async function updateRevision(revisionId, input, db = null) {
  ...
  const result = await queryWithDb(
    ...
    [
      ...
      input.contentFingerprint ?? current.contentFingerprint,
      input.supersededByRevisionId ?? current.supersededByRevisionId,
...
export async function listRecentAuditEventsByKeys({ eventKeys = [], since, limit = 8 } = {}, db = null) {
  const normalizedKeys = Array.isArray(eventKeys)
    ? eventKeys.filter((key) => typeof key === "string" && key)
    : [];
  const normalizedLimit = Math.max(1, Math.min(Number(limit) || 8, 50));

  if (normalizedKeys.length === 0 || !(since instanceof Date) || Number.isNaN(since.getTime())) {
    return [];
  }
  ...
  const result = await queryWithDb(
    ...
    `SELECT
       a.*,
       e.entity_type,
       r.payload AS revision_payload,
       u.display_name AS actor_display_name,
       u.username AS actor_username
     FROM audit_events a
     LEFT JOIN content_entities e ON e.id = a.entity_id
     LEFT JOIN content_revisions r ON r.id = a.revision_id
     LEFT JOIN app_users u ON u.id = a.actor_user_id
     WHERE a.event_key = ANY($1::text[])
       AND a.created_at >= $2
     ORDER BY a.created_at DESC
     LIMIT $3`,
    [normalizedKeys, since, normalizedLimit]
  );

  return result.rows.map(mapAuditEventRow);
}

export async function insertDestructiveForensicEvent(input, db = null) {
  await queryWithDb(
    db,
    `INSERT INTO destructive_forensic_events (
      id,
      correlation_id,
      operation_kind,
      outcome,
    ...
    [
      ...
      input.rootEntityType ?? null,
      input.rootEntityLabel ?? null,
      input.targetEntityId ?? null,
      input.targetEntityType ?? null,
      input.targetEntityLabel ?? null,
...
export async function listDestructiveForensicEvents(options = {}, db = null) {
  const clauses = [];
  const params = [];

  if (options.operationKind) {
    params.push(options.operationKind);
    clauses.push(`operation_kind = $${params.length}`);
  }
  ...
  const result = await queryWithDb(
    db,
    `SELECT *
     FROM destructive_forensic_events
     ${whereClause}
  ...
  return result.rows.map(mapDestructiveForensicEventRow);
}

export async function listDestructiveForensicEventsForEntity(entityId, options = {}, db = null) {
  ...
  const result = await query(
    `SELECT e.entity_type, e.id AS entity_id, r.*
     ...
     WHERE (r.state = 'review'
        OR (r.state = 'draft' AND r.owner_approval_status = 'rejected'))
...
export async function listActiveReviewRevisionsForEntity(entityId, db = null) {
  const result = await queryWithDb(
    db,
    `SELECT *
     FROM content_revisions
...
export async function findLatestSupersededRevisionForRevision(revisionId, db = null) {
  const result = await queryWithDb(
    db,
    `SELECT *
     FROM content_revisions
     WHERE superseded_by_revision_id = $1
     ORDER BY submitted_at DESC NULLS LAST, updated_at DESC
     LIMIT 1`,
    [revisionId]
  );
...
export async function findPublishObligationById(obligationId, db = null) {
  const result = await queryWithDb(
    db,
    "SELECT * FROM publish_obligations WHERE id = $1 LIMIT 1",
    [obligationId]
  );
...
export async function deleteEntityById(entityId, db = null) {
  const result = await queryWithDb(
    db,
    "DELETE FROM content_entities WHERE id = $1 RETURNING *",
    [entityId]
  );
...
export async function clearEntityActivePublishedRevision(entityId, userId, db = null, options = {}) {
  const expectedActivePublishedRevisionId = typeof options.expectedActivePublishedRevisionId === "string"
    && options.expectedActivePublishedRevisionId
    ? options.expectedActivePublishedRevisionId
    : null;
  const params = expectedActivePublishedRevisionId
    ? [entityId, userId, expectedActivePublishedRevisionId]
    : [entityId, userId];
  const expectedRevisionPredicate = expectedActivePublishedRevisionId
    ? "AND active_published_revision_id = $3"
    : "";
  const result = await queryWithDb(
    db,
    `UPDATE content_entities
...
export async function updateEntityCreationOrigin(entityId, creationOrigin, userId, db = null) {
  const result = await queryWithDb(
    db,
    `UPDATE content_entities
     SET creation_origin = $2,
         updated_by = $3,
         updated_at = NOW()
     WHERE id = $1
     RETURNING *`,
    [entityId, creationOrigin ?? null, userId]
  );

  return mapEntityRow(result.rows[0]);
}

export async function markEntityForRemoval(entityId, actorUserId, note = null, db = null) {
  const result = await queryWithDb(
    db,
    `UPDATE content_entities
     SET marked_for_removal_at = NOW(),
         marked_for_removal_by = $2,
         removal_note = $3,
         updated_by = $2,
         updated_at = NOW()
     WHERE id = $1
       AND marked_for_removal_at IS NULL
     RETURNING *`,
    [entityId, actorUserId, note ?? null]
  );

  return mapEntityRow(result.rows[0]);
}

export async function clearEntityRemovalMark(entityId, actorUserId, db = null) {
  const result = await queryWithDb(
    db,
    `UPDATE content_entities
     SET marked_for_removal_at = NULL,
