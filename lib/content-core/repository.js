...
export async function findEntityById(entityId) {
  const result = await query("SELECT * FROM content_entities WHERE id = $1", [entityId]);
  return mapEntityRow(result.rows[0]);
}

export async function findEntityByTypeSingleton(entityType) {
  const result = await query("SELECT * FROM content_entities WHERE entity_type = $1", [entityType]);
  return mapEntityRow(result.rows[0]);
}

export async function createEntity(entityType, userId) {
...
export async function updateEntityTimestamps(entityId, userId, activePublishedRevisionId = null) {
  const result = await query(
...
export async function findLatestRevision(entityId) {
  const result = await query(
...
export async function findDraftRevision(entityId) {
  const result = await query(
...
export async function findRevisionById(revisionId) {
  const result = await query("SELECT * FROM content_revisions WHERE id = $1", [revisionId]);
  return mapRevisionRow(result.rows[0]);
}

export async function findActivePublishedRevision(entityId) {
  const result = await query(
    `SELECT r.*
...
export async function listRevisionsForEntity(entityId) {
  const result = await query(
...
export async function updateRevision(revisionId, input) {
  const current = await findRevisionById(revisionId);
  const result = await query(
...
export async function listEntitiesWithLatestRevision(entityType) {
  const result = await query(
    `SELECT
       e.*,
       r.id AS revision_id,
...
export async function getEntityAggregate(entityId) {
  ...
  const revisions = await listRevisionsForEntity(entityId);
  const activePublishedRevision = entity.activePublishedRevisionId
    ? revisions.find((revision) => revision.id === entity.activePublishedRevisionId) ?? await findActivePublishedRevision(entityId)
    : null;
...
export async function listPublishedEntities(entityType) {
  const result = await query(
...
export async function findPublishedBySlug(entityType, slug) {
  const result = await query(
...
export async function findPublishedPageByPageType(pageType) {
  const result = await query(
    `SELECT e.id AS entity_id, r.*
     FROM content_entities e
     JOIN content_revisions r ON r.id = e.active_published_revision_id
...
export async function findPublishedPageTypeCollision(pageType, excludedEntityId) {
  const result = await query(
    `SELECT e.id
     FROM content_entities e
     JOIN content_revisions r ON r.id = e.active_published_revision_id
...
export async function findEntityByPublishedSlugCollision(entityType, slug, excludedEntityId) {
  const result = await query(
    `SELECT e.id
     FROM content_entities e
     JOIN content_revisions r ON r.id = e.active_published_revision_id
...
export async function listAuditEventsForEntity(entityId) {
  const result = await query(
...
export async function insertPublishObligation(input) {
  await query(
...
export async function listPublishObligations(entityId) {
  const result = await query(
...
export async function markPublishObligationCompleted(obligationId) {
  await query("UPDATE publish_obligations SET status = 'completed' WHERE id = $1", [obligationId]);
}
...
export async function updateUserActiveState(userId, active) {
  const result = await query(
    `UPDATE app_users
...
export async function updateUserRecord(userId, input) {
  const result = await query(
    `UPDATE app_users
     SET username = $2,
         display_name = $3,
