...
function normalizeServiceScopeDisplayMode(value) {
  const mode = asString(value);
  return Object.values(SERVICE_SCOPE_DISPLAY_MODES).includes(mode)
    ? mode
...
function normalizeSourceRefs(input = {}) {
  ...
  const serviceIds = asList(sourceRefs.serviceIds ?? input.serviceIds);
  ...
  switch (pageType) {
    case PAGE_TYPES.HOME:
      return [
        ...
        { type: PAGE_SECTION_TYPES.SERVICE_LIST, order: 1, title: "", body: "", serviceIds: [] },
  ...
  switch (section.type) {
    ...
    case PAGE_SECTION_TYPES.SERVICE_LIST:
      return {
        ...
        title: asString(section.title),
        body: asString(section.body),
        serviceIds: asList(section.serviceIds)
      };
  ...
  return sections.map((section) => {
    switch (section.type) {
      ...
      case PAGE_SECTION_TYPES.SERVICE_LIST:
        return {
          ...section,
          body: asString(input.serviceListBody),
          serviceIds: sourceRefs.serviceIds
        };
      case PAGE_SECTION_TYPES.SERVICE_SCOPE:
        return {
          ...section,
...
function buildPageBlocksFromSections({ input = {}, pageType, title, intro, primaryMediaAssetId, sections = [], sourceRefs = {}, mediaSettings = {} }) {
  ...
  const serviceList = sectionByType(sections, PAGE_SECTION_TYPES.SERVICE_LIST);
  ...
  if (pageType === PAGE_TYPES.HOME || pageType === PAGE_TYPES.ABOUT || pageType === PAGE_TYPES.CONTACTS) {
  ...
  const serviceListIds = serviceList?.serviceIds?.length
    ? serviceList.serviceIds
    : (sourceRefs.serviceIds?.length ? sourceRefs.serviceIds : (sourceRefs.primaryServiceId ? [sourceRefs.primaryServiceId] : []));

  if (serviceListIds.length > 0) {
    blocks.push({
      type: BLOCK_TYPES.SERVICE_LIST,
      order: blocks.length,
      title: serviceList?.title || (pageType === PAGE_TYPES.HOME ? "" : PAGE_BLOCK_DEFAULTS.relatedServicesTitle),
      serviceIds: serviceListIds
    });
  }
    ...
    pageType === PAGE_TYPES.HOME
      ...
      : pageType === PAGE_TYPES.ABOUT
        ...
        : pageType === PAGE_TYPES.CONTACTS
          ? "contacts"
          : deriveSlug(input.slug, input.seedSlug, title, h1) || "page";
...
export function normalizeEntityInput(entityType, input) {
  switch (entityType) {
    ...
    case ENTITY_TYPES.SERVICE:
      return validatePayload(entityType, {
        slug: deriveSlug(input.slug, input.title, input.h1),
        title: input.title,
        h1: input.h1,
        ...
        serviceScopeDisplayMode: normalizeServiceScopeDisplayMode(input.serviceScopeDisplayMode),
        serviceArea: input.serviceArea,
    ...
    case ENTITY_TYPES.EQUIPMENT:
      return validatePayload(entityType, {
        slug: deriveSlug(input.slug, input.title, input.equipmentType),
    ...
    case ENTITY_TYPES.CASE:
      return validatePayload(entityType, {
        slug: deriveSlug(input.slug, input.title),
        title: input.title,
