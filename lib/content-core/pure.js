  ...
  switch (pageType) {
    case PAGE_TYPES.HOME:
      return [
        ...
        { type: PAGE_SECTION_TYPES.SERVICE_LIST, order: 2, title: PAGE_BLOCK_DEFAULTS.relatedServicesTitle, body: "", serviceIds: [] },
        { type: PAGE_SECTION_TYPES.PROOF_CASES, order: 3, title: PAGE_BLOCK_DEFAULTS.relatedCasesTitle, caseIds: [], galleryIds: [] },
        { type: PAGE_SECTION_TYPES.CTA, order: 4, title: DEFAULT_CTA_COPY.title, body: "", ctaLabel: DEFAULT_CTA_COPY.label }
      ];
...
function buildPageBlocksFromSections({ input = {}, pageType, title, intro, primaryMediaAssetId, sections = [], sourceRefs = {}, mediaSettings = {} }) {
  ...
  if (serviceListIds.length > 0) {
    blocks.push({
      ...
      order: blocks.length,
      title: serviceList?.title || PAGE_BLOCK_DEFAULTS.relatedServicesTitle,
