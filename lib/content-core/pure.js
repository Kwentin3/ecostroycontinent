...
function buildPagePayload(input) {
  ...
  if ((input.serviceIds ?? []).length > 0) {
    blocks.push({
      type: "service_list",
      order: 2,
      title: "РЎРІСЏР·Р°РЅРЅС‹Рµ СѓСЃР»СѓРіРё",
  ...
  if ((input.caseIds ?? []).length > 0) {
    blocks.push({
      type: "case_list",
      order: 3,
      title: "РЎРІСЏР·Р°РЅРЅС‹Рµ РєРµР№СЃС‹",
  ...
  if (pageType === PAGE_TYPES.CONTACTS) {
    ...
    blocks.push({
      type: "cta",
      order: 5,
      title: input.ctaTitle || "РЎРІСЏР¶РёС‚РµСЃСЊ СЃ РЅР°РјРё",
      body: input.ctaBody || "",
      ctaLabel: input.defaultBlockCtaLabel || "РЎРІСЏР·Р°С‚СЊСЃСЏ СЃ РЅР°РјРё",
      textEmphasisPreset: input.ctaTextEmphasisPreset ?? DEFAULT_LANDING_TEXT_EMPHASIS_PRESET,
      surfaceTone: input.ctaSurfaceTone ?? DEFAULT_LANDING_SURFACE_TONE
    });
  }
