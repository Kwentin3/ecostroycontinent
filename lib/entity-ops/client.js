...
async function parseRedirectResponse(response, baseUrl) {
  ...
  if (location.pathname === "/admin/no-access") {
    throw new Error("Admin route returned no-access.");
  }
...
export class EntityOpsAdminClient {
  ...
  async runPageWorkspaceAction(pageId, body) {
    const response = await this.request(`/api/admin/entities/page/${pageId}/workspace`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok || !payload.ok) {
      throw new Error(payload.error || payload.message || `Page workspace action failed with status ${response.status}.`);
    }

    return payload;
  }

  async submitRevisionForReview(revisionId, formData) {
    const response = await this.request(`/api/admin/revisions/${revisionId}/submit`, {
      method: "POST",
      body: formData
    });
  ...
  async approveOwnerAction(revisionId, formData) {
    const response = await this.request(`/api/admin/revisions/${revisionId}/owner-action`, {
      method: "POST",
      body: formData
    });
  ...
  async publishRevision(revisionId, formData) {
    const response = await this.request(`/api/admin/revisions/${revisionId}/publish`, {
      method: "POST",
      body: formData
    });
