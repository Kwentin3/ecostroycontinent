...
export function formatEntityOpsTextReport(report) {
  ...
  for (const item of report.items) {
    ...
    if (item.revisionId) {
      lines.push(`  revision: ${item.revisionId}`);
    }

    if (item.relationField) {
      lines.push(`  relation: ${item.relationField}`);
    }

    if (Array.isArray(item.resolvedIds) && item.resolvedIds.length > 0) {
      lines.push(`  resolvedIds: ${item.resolvedIds.join(", ")}`);
    }

    if (item.latestRevision?.state) {
      lines.push(`  latestState: ${item.latestRevision.state}`);
    }

    if (item.activePublishedRevision?.id) {
      lines.push(`  publishedRevision: ${item.activePublishedRevision.id}`);
    }
