...
import { SERVICE_SCOPE_DISPLAY_MODES } from "../content-core/content-types.js";
import {
...
export const SERVICE_LANDING_SECTION_REGISTRY = Object.freeze([
  ...
  },
  {
    ...
    required: true,
    sourceFields: ["serviceScope", "serviceScopeDisplayMode", "problemsSolved", "methods"]
  },
  {
    id: "related_equipment",
    label: "Связанная техника",
    renderTarget: "Блок готовых объектов техники",
    required: false,
    sourceFields: ["equipmentIds"]
  },
  {
    id: "related_cases",
...
export const serviceLandingCandidatePayloadSchema = z.object({
  ...
  serviceScopeDisplayMode: z.enum(Object.values(SERVICE_SCOPE_DISPLAY_MODES)).default(SERVICE_SCOPE_DISPLAY_MODES.DEFAULT),
  serviceArea: optionalString,
  serviceAreaNote: optionalString,
...
export const serviceLandingCandidateResponseJsonSchema = {
  ...
  properties: {
    ...
    serviceScopeDisplayMode: { type: "string", enum: Object.values(SERVICE_SCOPE_DISPLAY_MODES) },
  ...
  return SERVICE_LANDING_SECTION_REGISTRY.map((section) => {
    ...
    switch (section.id) {
      ...
      case "service_scope":
        ...
        break;
      case "related_equipment":
        present = toCompactList(payload.equipmentIds).length > 0;
        break;
      case "related_cases":
...
export async function requestServiceLandingCandidate(input = {}, deps = {}) {
  ...
  const payloadWithDisplayMode = serviceLandingCandidatePayloadSchema.parse({
    ...payload,
    serviceScopeDisplayMode: request.normalizedPayload.serviceScopeDisplayMode
  });
  const spec = buildServiceLandingCandidateSpec({
    ...
    variantKey: input.variantKey ?? "",
    sourceContextSummary: request.sourceContextSummary,
    payload: payloadWithDisplayMode
  });
