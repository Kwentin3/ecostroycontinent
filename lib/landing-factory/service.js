...
export const SERVICE_LANDING_SECTION_REGISTRY = Object.freeze([
  ...
  },
  {
    id: "primary_media",
    label: "Primary media",
    renderTarget: "Primary media section",
    required: false,
  ...
  },
  {
    id: "service_scope",
    label: "Service scope",
    renderTarget: "Scope and methods section",
    required: true,
  ...
  },
  {
    id: "related_cases",
    label: "Related cases",
    renderTarget: "Related case cards",
    required: false,
...
export function buildServiceLandingSourceContextSummary({
  ...
  return parts.join(" | ").slice(0, 700) || "service landing candidate";
}

export function buildServiceLandingCandidatePrompt({
  sourcePayload,
  ...
  const promptPacket = assemblePromptPacket({
    ...
    canonicalContext: {
      ...canonicalContext,
      sourceContextSummary: sourceContextSummary || "service landing candidate"
    },
    artifactContract: {
    ...
    actionSlices: actionSlices.length > 0
      ? actionSlices
      : [
          {
            ...
            content: [
              ...
              `Source context summary: ${sourceContextSummary || "service landing candidate"}`,
...
export function buildServiceLandingVerificationReport({
  ...
  if (candidateSpec?.routeFamily !== SERVICE_LANDING_ROUTE_FAMILY) {
    contractIssues.push(
      makeReportIssue(
        ...
        `Candidate route family must be '${SERVICE_LANDING_ROUTE_FAMILY}'.`,
        "routeFamily"
      )
    );
  }
  ...
  if (candidateSpec?.specVersion !== SERVICE_LANDING_SPEC_VERSION) {
    contractIssues.push(
      makeReportIssue(
        ...
        `Candidate spec version must be '${SERVICE_LANDING_SPEC_VERSION}'.`,
        "specVersion"
      )
    );
  }
  ...
  if (JSON.stringify(sections.map((section) => section.id)) !== JSON.stringify(projectedSections.map((section) => section.id))) {
    contractIssues.push(
      makeReportIssue(
        ...
        "Candidate sections must follow the deterministic service registry order.",
        "sections"
      )
    );
  }
    ...
    if (section.required && section.status === "missing") {
      sectionIssues.push(
        makeReportIssue(
          ...
          `missing_${section.id}`,
          `Required service section '${section.id}' is missing.`,
          section.id
        )
      );
    }
  ...
  if (!hasText(candidateSpec?.payload?.problemsSolved)) {
    claimWarnings.push(
      makeReportIssue(
        ...
        "The candidate does not explain which problems it solves."
      )
    );
  }
  ...
  if (!hasText(candidateSpec?.payload?.methods)) {
    claimWarnings.push(
      makeReportIssue(
        ...
        "The candidate does not describe how the work is done."
      )
    );
  }
  ...
  return {
    ...
    overallStatus,
    summary: hasBlocking
      ? "There are blocking service candidate issues."
      : hasWarnings
        ? "There are non-blocking service candidate warnings."
        : "Service candidate passed verification.",
    classResults,
    blockingIssues,
    warnings,
    hasBlocking,
