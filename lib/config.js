...
const envSchema = z.object({
  ...
  NEXT_PUBLIC_YANDEX_METRICA_ENABLED: z.string().default("false"),
  NEXT_PUBLIC_YANDEX_METRICA_COUNTER_ID: z.string().trim().default(""),
  ...
  MEDIA_S3_LOCAL_FALLBACK_ENABLED: z.string().default(""),
  MEDIA_PUBLIC_BASE_URL: z.string().default(""),
  MEDIA_DELIVERY_MODE: z.enum(["app_proxy", "cdn", "auto"]).default("app_proxy"),
  ...
  BOOTSTRAP_SUPERADMIN_DISPLAY_NAME: z.string().min(1).default("Системный суперадмин"),
...
export function getAppConfig() {
  if (!cachedConfig) {
    ...
    if (parsed.MEDIA_STORAGE_MODE === "s3") {
      ...
      if (parsed.MEDIA_DELIVERY_MODE !== "app_proxy" && !parsed.MEDIA_PUBLIC_BASE_URL.trim()) {
        missing.push("MEDIA_PUBLIC_BASE_URL");
      }
    ...
    cachedConfig = {
      ...
      yandexMetricaPublicEnabled: parseBooleanToken(parsed.NEXT_PUBLIC_YANDEX_METRICA_ENABLED),
      yandexMetricaPublicCounterId: parsed.NEXT_PUBLIC_YANDEX_METRICA_COUNTER_ID.trim(),
      ...
      mediaS3LocalFallbackEnabled: parseBooleanToken(parsed.MEDIA_S3_LOCAL_FALLBACK_ENABLED),
      mediaPublicBaseUrl: parsed.MEDIA_PUBLIC_BASE_URL.replace(/\/+$/, ""),
      mediaDeliveryMode: parsed.MEDIA_DELIVERY_MODE,
