...
} from "../content-ops/reference-readiness-contract.js";
...
export const landingWorkspaceCandidatePayloadSchema = z.object({
  pageType: z.enum([PAGE_TYPES.HOME, PAGE_TYPES.ABOUT, PAGE_TYPES.CONTACTS]).default(PAGE_TYPES.ABOUT),
  slug: nonEmptyString,
...
export const landingWorkspaceCandidateResponseJsonSchema = {
  ...
  properties: {
    pageType: { type: "string", enum: [PAGE_TYPES.HOME, PAGE_TYPES.ABOUT, PAGE_TYPES.CONTACTS] },
...
export const LANDING_WORKSPACE_BLOCK_REGISTRY = Object.freeze([
  { id: "landing_hero", label: "Главный блок", renderTarget: "Верхний блок страницы", required: true, sourceFields: ["title", "hero.headline", "hero.textEmphasisPreset", "hero.surfaceTone"] },
  { id: "media_strip", label: "Медиа-полоса", renderTarget: "Поддерживающая медиа-полоса", required: false, sourceFields: ["mediaAssetIds"] },
  { id: "service_cards", label: "Карточки услуг", renderTarget: "Карточки подтверждения услуг", required: false, sourceFields: ["serviceCardIds"] },
  { id: "case_cards", label: "Карточки кейсов", renderTarget: "Карточки подтверждения кейсов", required: false, sourceFields: ["caseCardIds"] },
  { id: "content_band", label: "Контент-блок", renderTarget: "Блок поддерживающего текста", required: false, sourceFields: ["contentBand.body", "contentBand.subtitle", "contentBand.textEmphasisPreset", "contentBand.surfaceTone"] },
  { id: "cta_band", label: "Блок призыва к действию", renderTarget: "Финальный блок призыва к действию", required: false, sourceFields: ["ctaVariant", "ctaBand.title", "ctaBand.body", "ctaBand.note", "ctaBand.textEmphasisPreset", "ctaBand.surfaceTone"] }
]);

export const LANDING_WORKSPACE_SHELL_REGION_REGISTRY = Object.freeze([
  { id: "landing_header", label: "Шапка лендинга", sourceField: "shellRegions.headerRef" },
  { id: "landing_footer", label: "Подвал лендинга", sourceField: "shellRegions.footerRef" }
]);
...
export const landingWorkspaceDraftResponseJsonSchema = {
  ...
  properties: {
    ...
    pageType: { type: "string", enum: [PAGE_TYPES.HOME, PAGE_TYPES.ABOUT, PAGE_TYPES.CONTACTS] },
    pageThemeKey: { type: "string", enum: LANDING_PAGE_THEME_KEYS },
    slug: { type: "string", minLength: 1 },
    title: { type: "string", minLength: 1 },
    hero: {
      type: "object",
      ...
      properties: {
        ...
        textEmphasisPreset: { type: "string", enum: LANDING_TEXT_EMPHASIS_PRESETS },
        surfaceTone: { type: "string", enum: LANDING_SURFACE_TONES }
      },
      required: ["headline", "body", "mediaAssetId", "textEmphasisPreset", "surfaceTone"]
    },
    ...
    contentBand: {
      ...
      required: ["body", "subtitle", "textEmphasisPreset", "surfaceTone"]
    },
    ...
    ctaBand: {
      ...
      required: ["title", "body", "note", "textEmphasisPreset", "surfaceTone"]
    },
    shellRegions: {
      type: "object",
      additionalProperties: false,
...
function buildStageAToneDefaults(input = null) {
  const source = input && typeof input === "object" ? input : {};

  return {
    textEmphasisPreset: asString(source.textEmphasisPreset) || DEFAULT_LANDING_TEXT_EMPHASIS_PRESET,
    surfaceTone: asString(source.surfaceTone) || DEFAULT_LANDING_SURFACE_TONE
  };
}
...
function buildDraftFromLegacyPayload(payload = {}) {
  ...
  return {
    ...
    pageThemeKey: asString(payload.pageThemeKey) || DEFAULT_LANDING_PAGE_THEME_KEY,
    ...
    hero: {
      ...
      ...buildStageAToneDefaults(heroBlock)
    },
    ...
    contentBand: {
      ...
      subtitle: "",
      ...buildStageAToneDefaults(richTextBlock)
    },
    ...
    ctaBand: {
      ...
      ...buildStageAToneDefaults(ctaBlock ?? contactBlock)
    },
...
export function buildLandingWorkspacePagePayload(candidatePayload = {}) {
  ...
  return {
    ...
    pageThemeKey: draft.pageThemeKey,
    slug: draft.slug,
    title: draft.title,
    h1: draft.hero.headline,
    intro: draft.hero.body,
    heroTextEmphasisPreset: draft.hero.textEmphasisPreset,
    heroSurfaceTone: draft.hero.surfaceTone,
    body: draft.contentBand.body,
    contentBandTextEmphasisPreset: draft.contentBand.textEmphasisPreset,
    contentBandSurfaceTone: draft.contentBand.surfaceTone,
    ...
    ctaTextEmphasisPreset: draft.ctaBand.textEmphasisPreset,
    ctaSurfaceTone: draft.ctaBand.surfaceTone,
...
export function projectLandingWorkspaceCandidatePayload(payload = {}) {
  ...
  if (looksCanonical) {
    return landingWorkspaceDraftSchema.parse({
      ...
      pageThemeKey: payload.pageThemeKey,
      slug: payload.slug,
      title: payload.title,
      hero: payload.hero ?? {},
...
function collectReadinessIssues(readiness = null) {
  ...
  return {
    ...
    reference: results.filter((result) => isBlockingReferenceReadinessCode(result.code)),
    editorial: results.filter((result) => !isBlockingReferenceReadinessCode(result.code))
  };
}

function buildTextBearingBlockAppearance(payload) {
  return [
    {
      blockId: "landing_hero",
      fieldPrefix: "hero",
      textEmphasisPreset: payload.hero?.textEmphasisPreset ?? DEFAULT_LANDING_TEXT_EMPHASIS_PRESET,
      surfaceTone: payload.hero?.surfaceTone ?? DEFAULT_LANDING_SURFACE_TONE,
      hasMeaningfulCopy: hasText(payload.hero?.headline) || hasText(payload.hero?.body)
    },
    {
      blockId: "content_band",
      fieldPrefix: "contentBand",
      textEmphasisPreset: payload.contentBand?.textEmphasisPreset ?? DEFAULT_LANDING_TEXT_EMPHASIS_PRESET,
      surfaceTone: payload.contentBand?.surfaceTone ?? DEFAULT_LANDING_SURFACE_TONE,
      hasMeaningfulCopy: hasText(payload.contentBand?.body) || hasText(payload.contentBand?.subtitle)
    },
    {
      blockId: "cta_band",
      fieldPrefix: "ctaBand",
      textEmphasisPreset: payload.ctaBand?.textEmphasisPreset ?? DEFAULT_LANDING_TEXT_EMPHASIS_PRESET,
      surfaceTone: payload.ctaBand?.surfaceTone ?? DEFAULT_LANDING_SURFACE_TONE,
      hasMeaningfulCopy: hasText(payload.ctaBand?.title) || hasText(payload.ctaBand?.body) || hasText(payload.ctaBand?.note) || hasText(payload.ctaVariant)
    }
  ];
}

function collectVisualReadabilityIssues(candidatePayload) {
  const theme = resolveLandingPageTheme(candidatePayload.pageThemeKey);
  const issues = [];
  const warnings = [];
  const appearances = buildTextBearingBlockAppearance(candidatePayload);

  for (const appearance of appearances) {
    const semantics = resolveLandingSurfaceSemantics(candidatePayload.pageThemeKey, appearance.surfaceTone);
    ...
    if (!semantics) {
      issues.push(
        makeReportIssue(
          "blocking",
          "visual/readability",
          `missing_surface_semantics_${appearance.blockId}`,
          `Theme ${candidatePayload.pageThemeKey} does not resolve surfaceTone ${appearance.surfaceTone} for ${appearance.blockId}.`,
          `${appearance.fieldPrefix}.surfaceTone`
        )
      );
      continue;
    }

    if (!appearance.hasMeaningfulCopy) {
      continue;
    }

    if (semantics.contrastStatus === "blocking") {
      issues.push(
        makeReportIssue(
          "blocking",
          "visual/readability",
          `contrast_blocked_${appearance.blockId}`,
          `Resolved ${appearance.blockId} appearance is not readable enough for publish.`,
          `${appearance.fieldPrefix}.surfaceTone`
        )
      );
    }

    if (semantics.contrastStatus === "warning") {
      warnings.push(
        makeReportIssue(
          "warning",
          "visual/readability",
          `contrast_warning_${appearance.blockId}`,
          `Resolved ${appearance.blockId} appearance is readable, but should be reviewed for contrast and visual calm.`,
          `${appearance.fieldPrefix}.surfaceTone`
        )
      );
    }
  ...
  const emphasizedBlocks = appearances.filter((appearance) => appearance.surfaceTone === "emphasis" && appearance.hasMeaningfulCopy);
  if (emphasizedBlocks.length >= 3) {
    warnings.push(
      makeReportIssue(
        "warning",
        "visual/readability",
        "overused_emphasis_surface",
        "Too many text-bearing blocks use emphasis surfaceTone at once; page rhythm may feel visually noisy.",
        "pageThemeKey"
      )
    );
  }
  ...
  return {
    issues,
    warnings,
    themeKey: candidatePayload.pageThemeKey,
    themeLabel: theme.label
  };
}

function collectOrderedProofInputIssues(candidatePayload) {
  const issues = [];
  const orderedProofRefs = [
    { field: "mediaAssetIds", blockId: "media_strip", values: candidatePayload.mediaAssetIds ?? [] },
    { field: "serviceCardIds", blockId: "service_cards", values: candidatePayload.serviceCardIds ?? [] },
    { field: "caseCardIds", blockId: "case_cards", values: candidatePayload.caseCardIds ?? [] }
  ];

  for (const group of orderedProofRefs) {
    const normalized = toCompactList(group.values);
    if (new Set(normalized).size !== normalized.length) {
      issues.push(
        makeReportIssue(
          "blocking",
          "structural/schema",
          `duplicate_ordered_refs_${group.blockId}`,
          `Ordered refs for ${group.blockId} must stay unique so render order remains deterministic.`,
          group.field
        )
      );
    }
...
export function buildLandingWorkspaceVerificationReport({
  ...
  const orderedProofInputIssues = collectOrderedProofInputIssues(candidatePayload);
  const visualReadability = collectVisualReadabilityIssues(candidatePayload);
  ...
  const classResults = [
    ...
    },
    {
      ...
      issues: readinessBuckets.reference.map((result) =>
        makeReportIssue(getReferenceReadinessSeverity(result.code), "reference/truth", result.code, result.message, result.field)
      )
    },
    {
    ...
    {
      classId: "visual/readability",
      status: visualReadability.issues.length > 0
        ? "blocked"
        : visualReadability.warnings.length > 0
          ? "warning"
          : "pass",
      issues: [...visualReadability.issues, ...visualReadability.warnings]
    },
    {
      classId: "claim/risk",
  ...
  return {
    ...
    sections,
    shellRegions,
    pageThemeKey: candidatePayload.pageThemeKey,
    visualSemantics: {
      pageThemeKey: candidatePayload.pageThemeKey,
      themeLabel: visualReadability.themeLabel
    },
    renderCompatible,
    publishReady,
    approvalEligible,
