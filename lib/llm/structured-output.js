...
export function normalizeStructuredJsonText(text, context = {}) {
  ...
  if (!trimmed) {
    throw createLlmError({
      ...
      message: "Провайдер вернул пустой structured-output ответ.",
      retryable: false,
  ...
  try {
    ...
    throw createLlmError({
      ...
      message: "Ответ провайдера не удалось разобрать как JSON.",
      retryable: false,
      stage: "structured_output",
      details: context
    });
...
export function validateStructuredArtifact(schema, artifact, context = {}) {
  if (!schema || typeof schema.safeParse !== "function") {
    throw createLlmError({
      ...
      message: "Для structured-output запроса требуется локальная схема валидации.",
      retryable: false,
      stage: "configuration",
      details: context
    });
  ...
  if (!validation.success) {
    throw createLlmError({
      ...
      message: "Нормализованный артефакт не прошёл локальную валидацию.",
      retryable: false,
      stage: "validation",
      details: {
        ...context,
