...
export async function requestStructuredArtifact(request, deps = {}) {
  ...
  if (requestIssues.length > 0) {
    const error = createLlmError({
      ...
      message: "The structured-output request is incomplete.",
      retryable: false,
      stage: "configuration",
      details: {
  ...
  if (configSnapshot.state !== LLM_CONFIG_STATES.CONFIGURED) {
    const error = createLlmConfigFailure(configSnapshot, "LLM configuration is incomplete or unsupported.");
    return createLlmFailureResult(base, error, patchForError(error));
  }
  ...
  if (configSnapshot.providerId !== LLM_PROVIDER_IDS.GEMINI) {
    const error = createLlmError({
      ...
      message: `Provider '${configSnapshot.providerId}' is not supported by the current baseline.`,
      retryable: false,
      stage: "configuration",
      details: {
        providerId: configSnapshot.providerId,
  ...
  try {
    ...
    const normalizedError = normalizeLlmError(error, {
      ...
      message: "The structured-output request failed.",
      retryable: false,
      stage: "provider"
    });
