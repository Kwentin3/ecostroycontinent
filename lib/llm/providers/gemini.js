...
function buildGenerationConfig(responseJsonSchema, configSnapshot) {
  const generationConfig = {
    ...
    responseJsonSchema,
    temperature: 0,
    // Service landing candidates are larger than the tiny diagnostic probe and
    // need enough headroom to return the full structured artifact without truncation.
    ...
    async requestStructuredArtifact({
      ...
      if (!responseJsonSchema) {
        throw createLlmError({
          ...
          message: "Для structured-output запроса требуется схема ответа провайдера.",
          retryable: false,
          stage: "configuration",
          details: {
      ...
      if (response.status < 200 || response.status >= 300) {
        throw createLlmError({
          ...
          message: "Провайдер Gemini отклонил запрос.",
      ...
      try {
        ...
        throw createLlmError({
          ...
          message: "Провайдер Gemini вернул тело ответа не в формате JSON.",
          retryable: false,
          stage: "provider",
          details: {
            providerId: "gemini",
      ...
      if (payload?.error) {
        throw createLlmError({
          ...
          message: typeof payload.error.message === "string" && payload.error.message.trim()
            ...
            : "Провайдер Gemini вернул ошибку.",
      ...
      if (!text) {
        throw createLlmError({
          ...
          message: "Ответ провайдера Gemini не содержал структурированного текстового контента.",
          retryable: false,
          stage: "provider",
          details: {
            providerId: "gemini",
