...
import { getPublicMediaAssetDelivery } from "./public-media-url.js";
...
export async function getPublishedMediaAssets() {
  ...
  return rows.map((row) => {
    const asset = unwrapPublishedRow(row);
    const delivery = getPublicMediaAssetDelivery(asset);

    return {
      ...asset,
      previewUrl: delivery.url,
      previewFallbackUrl: delivery.fallbackUrl,
      previewDeliveryMode: delivery.mode
    };
  });
}
...
export async function getPublishedHomePage() {
  return getPublishedPage(PAGE_TYPES.HOME);
}
