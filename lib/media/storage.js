...
export function getMediaDeliveryUrl({ entityId, storageKey }, config = getAppConfig()) {
  if (config.mediaStorageMode === "s3" && config.mediaPublicBaseUrl && (entityId || storageKey)) {
    // Public media URLs stay on the published app surface so the app can
    // serve S3-backed bytes directly without exposing storage internals.
    return `${normalizeBaseUrl(config.mediaPublicBaseUrl)}/${encodeURIComponent(entityId || storageKey)}`;
  }
