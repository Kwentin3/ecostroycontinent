...
export const PUBLIC_MEDIA_CDN_REDIRECT_CACHE_CONTROL = "public, max-age=300";
export const PUBLIC_MEDIA_APP_PROXY_CACHE_CONTROL = "public, max-age=3600";

// Sticky canon: SQL owns media metadata, S3 owns binaries, CDN is delivery.
// Markup may point directly at CDN in CDN-capable modes; app routes remain
// fallback delivery boundaries and never become editorial media truth.
...
export function resolvePublicMediaMarkupDelivery({ asset, config } = {}) {
  const appProxyUrl = getAppProxyMediaUrl(asset?.entityId);
  const cdnUrl = getCdnMediaUrl({ storageKey: asset?.storageKey }, config);
  const mode = config?.mediaDeliveryMode || "app_proxy";

  if ((mode === "cdn" || mode === "auto") && cdnUrl) {
    return { mode: "cdn", url: cdnUrl, fallbackUrl: appProxyUrl };
  }
...
export function createPublicMediaRedirectResponse(url) {
  return new Response(null, {
    status: 302,
    headers: {
      Location: url,
      "cache-control": PUBLIC_MEDIA_CDN_REDIRECT_CACHE_CONTROL
    }
  });
}
