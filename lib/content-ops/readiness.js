...
import { buildServiceAreaReadinessResults } from "../content-core/geography.js";
import { listMarkedReferenceConflicts } from "../content-core/removal-references.js";
import { isLegacyNonLaunchPageType, isStrictPageOwnershipLaunchMode } from "../public-launch/ownership.js";
import { makeReferenceReadinessResult } from "./reference-readiness-contract.js";

// Publish-time cross-domain discipline lives here.
// When relation fields change, update this file together with schemas.js,
// entity-references.js, and public-content.js per CONTENT_DOMAIN_INTERACTION_CONTRACT_v1.
...
function makeReferenceResult(code, message, field = null) {
  return makeReferenceReadinessResult({ code, message, field });
}
...
async function validateOptionalPublishedRef(entityId, field, missingCode, unpublishedCode, missingMessage, unpublishedMessage) {
  ...
  if (!exists) {
    return [makeReferenceResult(missingCode, missingMessage, field)];
  }

  const published = await publishedRefsExist([entityId]);

  if (!published) {
    return [makeReferenceResult(unpublishedCode, unpublishedMessage, field)];
  }
...
async function evaluateGlobalSettings(payload) {
  ...
  if (payload.contactTruthConfirmed) {
    if (!hasText(payload.primaryPhone) && !hasText(payload.publicEmail)) {
      results.push(makeResult("blocking", "missing_public_contact", "При подтвержденных контактах нужен телефон или публичная почта.", "publicEmail"));
    }
...
async function evaluateGallery(payload) {
  ...
  if (!hasText(payload.title)) {
    results.push(makeResult("blocking", "missing_gallery_title", "Для галереи нужно название.", "title"));
  }
  ...
  if ((payload.assetIds ?? []).length === 0) {
    ...
    results.push(makeReferenceResult("invalid_asset_refs", "В галерее есть ссылки на отсутствующие медиафайлы.", "assetIds"));
  } else if (!(await publishedRefsExist(payload.assetIds))) {
    results.push(makeReferenceResult("unpublished_asset_refs", "В галерее можно использовать только опубликованные медиафайлы.", "assetIds"));
  }
...
async function evaluateService(entityId, payload, globalSettingsPayload = null) {
  const results = [];
  ...
  results.push(...buildServiceAreaReadinessResults({
    service: payload,
    globalSettings: globalSettingsPayload
  }));

  const proofPathCount =
    (payload.equipmentIds?.length ?? 0)
    +
  ...
  if ((payload.equipmentIds ?? []).length > 0) {
    if (!(await refsExist(payload.equipmentIds))) {
      results.push(makeReferenceResult("invalid_equipment_refs", "В услуге есть ссылки на отсутствующую технику.", "equipmentIds"));
    } else if (!(await publishedRefsExist(payload.equipmentIds))) {
      results.push(makeReferenceResult("unpublished_equipment_refs", "В услуге можно ссылаться только на опубликованную технику.", "equipmentIds"));
    }
  }

  if ((payload.relatedCaseIds ?? []).length > 0) {
    if (!(await refsExist(payload.relatedCaseIds))) {
      results.push(makeReferenceResult("invalid_case_refs", "В услуге есть ссылки на отсутствующие кейсы.", "relatedCaseIds"));
    } else if (!(await publishedRefsExist(payload.relatedCaseIds))) {
      results.push(makeReferenceResult("unpublished_case_refs", "В услуге можно ссылаться только на опубликованные кейсы.", "relatedCaseIds"));
    }
  }

  if ((payload.galleryIds ?? []).length > 0) {
    if (!(await refsExist(payload.galleryIds))) {
      results.push(makeReferenceResult("invalid_gallery_refs", "В услуге есть ссылки на отсутствующие галереи.", "galleryIds"));
    } else if (!(await publishedRefsExist(payload.galleryIds))) {
      results.push(makeReferenceResult("unpublished_gallery_refs", "В услуге можно ссылаться только на опубликованные галереи.", "galleryIds"));
    }
  }
...
async function evaluateCase(entityId, payload) {
  ...
  if ((payload.galleryIds ?? []).length > 0) {
    if (!(await refsExist(payload.galleryIds))) {
      results.push(makeReferenceResult("invalid_gallery_refs", "В кейсе есть ссылки на отсутствующие галереи.", "galleryIds"));
    } else if (!(await publishedRefsExist(payload.galleryIds))) {
      results.push(makeReferenceResult("unpublished_gallery_refs", "В кейсе можно ссылаться только на опубликованные галереи.", "galleryIds"));
    }
  }

  if ((payload.serviceIds ?? []).length > 0) {
    if (!(await refsExist(payload.serviceIds))) {
      results.push(makeReferenceResult("invalid_service_refs", "В кейсе есть ссылки на отсутствующие услуги.", "serviceIds"));
    } else if (!(await publishedRefsExist(payload.serviceIds))) {
      results.push(makeReferenceResult("unpublished_service_refs", "В кейсе можно ссылаться только на опубликованные услуги.", "serviceIds"));
    }
  }

  if ((payload.equipmentIds ?? []).length > 0) {
    if (!(await refsExist(payload.equipmentIds))) {
      results.push(makeReferenceResult("invalid_equipment_refs", "В кейсе есть ссылки на отсутствующую технику.", "equipmentIds"));
    } else if (!(await publishedRefsExist(payload.equipmentIds))) {
      results.push(makeReferenceResult("unpublished_equipment_refs", "В кейсе можно ссылаться только на опубликованную технику.", "equipmentIds"));
    }
  }
...
async function evaluatePage(entityId, payload) {
  ...
  if (payload.pageType === PAGE_TYPES.HOME && payload.slug !== "home") {
    results.push(makeResult("blocking", "home_slug_fixed", "У страницы «Главная» служебный адрес должен оставаться home.", "slug"));
  }
  ...
  if (payload.pageType === PAGE_TYPES.HOME || payload.pageType === PAGE_TYPES.ABOUT || payload.pageType === PAGE_TYPES.CONTACTS) {
  ...
  const requiredSectionTypes = payload.pageType === PAGE_TYPES.HOME
    ? [
    ...
    : payload.pageType === PAGE_TYPES.SERVICE_LANDING
    ? [
        PAGE_SECTION_TYPES.HERO_OFFER,
...
export async function evaluateReadiness({ entity, revision, globalSettingsRevision }) {
  ...
  switch (entity.entityType) {
    ...
    case ENTITY_TYPES.SERVICE:
      results.push(...(await evaluateService(entity.id, revision.payload, settingsRevision?.payload || null)));
      break;
  ...
  const markedReferenceConflicts = await listMarkedReferenceConflicts(entity.entityType, revision.payload);

  for (const conflict of markedReferenceConflicts) {
    results.push(
      makeResult(
        "blocking",
        "marked_for_removal_ref",
        `Ссылка указывает на объект, помеченный на удаление: ${conflict.label}.`,
        conflict.field || null
      )
    );
  }
  ...
  if (entity.entityType === ENTITY_TYPES.PAGE && revision.payload.pageType === PAGE_TYPES.CONTACTS) {
    ...
    if (!settingsRevision?.payload?.primaryPhone && !settingsRevision?.payload?.publicEmail) {
      results.push(makeResult("blocking", "contacts_missing_public_contact", "Для страницы «Контакты» нужен телефон или публичная почта из глобальных настроек."));
    }
