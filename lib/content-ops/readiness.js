...
async function evaluateService(entityId, payload, globalSettingsPayload = null) {
  ...
  if ((payload.equipmentIds ?? []).length > 0) {
    if (!(await refsExist(payload.equipmentIds))) {
      results.push(makeResult("blocking", "invalid_equipment_refs", "В услуге есть ссылки на отсутствующую технику.", "equipmentIds"));
    } else if (!(await publishedRefsExist(payload.equipmentIds))) {
      results.push(makeResult("blocking", "unpublished_equipment_refs", "В услуге можно ссылаться только на опубликованную технику.", "equipmentIds"));
    }
  }
...
async function evaluateCase(entityId, payload) {
  ...
  if ((payload.equipmentIds ?? []).length > 0) {
    if (!(await refsExist(payload.equipmentIds))) {
      results.push(makeResult("blocking", "invalid_equipment_refs", "В кейсе есть ссылки на отсутствующую технику.", "equipmentIds"));
    } else if (!(await publishedRefsExist(payload.equipmentIds))) {
      results.push(makeResult("blocking", "unpublished_equipment_refs", "В кейсе можно ссылаться только на опубликованную технику.", "equipmentIds"));
    }
  }
