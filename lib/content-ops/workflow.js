import {
  findEntityById,
  findEntityByTypeSingleton,
  findRevisionById,
  findActivePublishedRevision,
  findEntityByPublishedSlugCollision,
  findPublishObligationById,
  findLatestSupersededRevisionForRevision,
  insertPublishObligation,
  listActiveReviewRevisionsForEntity,
  listReviewQueue,
  markPublishObligationCompleted,
  updateEntityTimestamps,
  updateRevision
} from "../content-core/repository.js";
import { AUDIT_EVENT_KEYS, ENTITY_TYPES, PREVIEW_STATUS } from "../content-core/content-types.js";
import { buildHumanReadableDiff } from "../content-core/diff.js";
import { computeContentFingerprint } from "../content-core/fingerprint.js";
import { withTransaction } from "../db/client.js";
import { evaluateReadiness } from "./readiness.js";
import { recordAuditEvent } from "./audit.js";
import { buildPublishFollowUp, hasSlugMutation } from "./publish-follow-up.js";

function revisionHasResolvedReviewOutcome(revision = {}) {
  return revision.state === "superseded" || revision.ownerApprovalStatus === "approved";
}

export function filterPendingReviewQueueItems(queue = []) {
  return queue.filter((item) => !revisionHasResolvedReviewOutcome(item?.revision));
}

function getRevisionContentFingerprint(revision = {}) {
  return revision.contentFingerprint || computeContentFingerprint(revision.payload);
}

function pickReviewMaterialTitle(payload = {}) {
  return [
    payload.title,
    payload.h1,
    payload.publicBrandName,
    payload.originalFilename,
    payload.caption,
    payload.slug
  ]
    .map((value) => (typeof value === "string" ? value.trim() : ""))
    .find(Boolean) || "";
}

function buildReviewAuditDetails(revision = {}, details = {}) {
  return {
    ...details,
    materialTitle: pickReviewMaterialTitle(revision.payload || {}),
    revisionNumber: revision.revisionNumber ?? null
  };
}

// Fingerprint is the review idempotency boundary: same content reuses the
// active request, changed content supersedes older active review requests.
export function resolveReviewSubmissionCandidate({ activeReviewRevisions = [], draftRevision }) {
  const contentFingerprint = getRevisionContentFingerprint(draftRevision);
  const duplicateRevision = activeReviewRevisions.find((revision) => {
    return getRevisionContentFingerprint(revision) === contentFingerprint;
  }) ?? null;

  if (duplicateRevision) {
    return {
      contentFingerprint,
      duplicateRevision,
      supersededRevisions: []
    };
  }

  return {
    contentFingerprint,
    duplicateRevision: null,
    supersededRevisions: activeReviewRevisions
  };
}

async function attachReviewContext(item) {
  const [previousReviewRevision, publishedRevision] = await Promise.all([
    findLatestSupersededRevisionForRevision(item.revision.id),
    findActivePublishedRevision(item.entityId)
  ]);

  return {
    ...item,
    reviewContext: {
      supersedesRevisionId: previousReviewRevision?.id ?? null,
      previousReviewDiffRows: previousReviewRevision
        ? buildHumanReadableDiff(item.entityType, previousReviewRevision.payload, item.revision.payload)
        : [],
      publishedRevisionId: publishedRevision?.id ?? null,
      publishedDiffRows: publishedRevision
        ? buildHumanReadableDiff(item.entityType, publishedRevision.payload, item.revision.payload)
        : []
    }
  };
}

async function getPublishedGlobalSettingsRevision() {
  const entity = await findEntityByTypeSingleton(ENTITY_TYPES.GLOBAL_SETTINGS);

  if (!entity?.activePublishedRevisionId) {
    return null;
  }

  return findRevisionById(entity.activePublishedRevisionId);
}

export async function getReviewQueue() {
  const queue = await listReviewQueue();

  // The review queue is only for items that still need review-lane attention.
  // Publication is allowed only after explicit approval, so legacy
  // `not_required` review rows stay visible instead of disappearing.
  const pendingQueue = filterPendingReviewQueueItems(queue);

  return Promise.all(pendingQueue.map((item) => attachReviewContext(item)));
}

export async function submitRevisionForReview({ revisionId, actorUserId, canRenderPreview = true, previewFailureReason = null }) {
  const revision = await findRevisionById(revisionId);

  if (!revision || revision.state !== "draft") {
    throw new Error("Только черновики можно отправлять на проверку.");
  }

  const entity = await findEntityById(revision.entityId);
  const globalSettingsRevision = entity.entityType === ENTITY_TYPES.GLOBAL_SETTINGS
    ? null
    : await getPublishedGlobalSettingsRevision();
  const readiness = await evaluateReadiness({ entity, revision, globalSettingsRevision });

  const hasStructuralReferenceFailure = readiness.results.some((result) => result.code.startsWith("invalid_"));

  if (hasStructuralReferenceFailure) {
    throw new Error("Сломанные связи не позволяют отправить версию на проверку.");
  }

  const nextPreviewStatus = canRenderPreview ? PREVIEW_STATUS.RENDERABLE : PREVIEW_STATUS.UNAVAILABLE;

  return withTransaction(async (db) => {
    const currentRevision = await findRevisionById(revisionId, db);

    if (!currentRevision || currentRevision.state !== "draft") {
      throw new Error("Только черновики можно отправлять на проверку.");
    }

    const activeReviewRevisions = await listActiveReviewRevisionsForEntity(entity.id, db);
    const reviewCandidate = resolveReviewSubmissionCandidate({
      activeReviewRevisions,
      draftRevision: currentRevision
    });

    if (reviewCandidate.duplicateRevision) {
      await recordAuditEvent({
        entityId: entity.id,
        revisionId: currentRevision.id,
        actorUserId,
        eventKey: AUDIT_EVENT_KEYS.REVIEW_DUPLICATE_REQUESTED,
        summary: "Такая версия уже находится на проверке.",
        details: buildReviewAuditDetails(currentRevision, {
          existingRevisionId: reviewCandidate.duplicateRevision.id
        })
      }, { db });

      return {
        entity,
        revision: reviewCandidate.duplicateRevision,
        readiness,
        submissionStatus: "duplicate",
        duplicateOfRevisionId: reviewCandidate.duplicateRevision.id,
        supersededRevisionIds: []
      };
    }

    const updated = await updateRevision(revisionId, {
      state: "review",
      ownerReviewRequired: true,
      submittedAt: new Date(),
      ownerApprovalStatus: "pending",
      previewStatus: nextPreviewStatus,
      previewFailureReason,
      contentFingerprint: reviewCandidate.contentFingerprint,
      reviewComment: "",
      userId: actorUserId
    }, db);

    for (const supersededRevision of reviewCandidate.supersededRevisions) {
      await updateRevision(supersededRevision.id, {
        state: "superseded",
        supersededByRevisionId: updated.id,
        userId: actorUserId
      }, db);

      await recordAuditEvent({
        entityId: entity.id,
        revisionId: supersededRevision.id,
        actorUserId,
        eventKey: AUDIT_EVENT_KEYS.REVIEW_SUPERSEDED,
        summary: "Заявка на проверку заменена новой версией.",
        details: buildReviewAuditDetails(supersededRevision, {
          supersededByRevisionId: updated.id
        })
      }, { db });
    }

    await recordAuditEvent({
      entityId: entity.id,
      revisionId: currentRevision.id,
      actorUserId,
      eventKey: AUDIT_EVENT_KEYS.REVIEW_REQUESTED,
      summary: "Черновик отправлен на проверку.",
      details: buildReviewAuditDetails(currentRevision, {
        readiness,
        previewStatus: nextPreviewStatus
      })
    }, { db });

    if (!canRenderPreview) {
      await recordAuditEvent({
        entityId: entity.id,
        revisionId: currentRevision.id,
        actorUserId: null,
        eventKey: AUDIT_EVENT_KEYS.PREVIEW_RENDER_FAILED,
        summary: "Предпросмотр версии оказался недоступен.",
        details: buildReviewAuditDetails(currentRevision, {
          previewFailureReason
        })
      }, { db });
    }

    await recordAuditEvent({
      entityId: entity.id,
      revisionId: currentRevision.id,
      actorUserId,
      eventKey: AUDIT_EVENT_KEYS.OWNER_REVIEW_REQUESTED,
      summary: "Версия отправлена на согласование владельцу.",
      details: buildReviewAuditDetails(currentRevision, {
        ownerReviewRequired: true,
        previousOwnerReviewRequired: Boolean(currentRevision.ownerReviewRequired)
      })
    }, { db });

    return {
      entity,
      revision: updated,
      readiness,
      submissionStatus: reviewCandidate.supersededRevisions.length > 0 ? "updated" : "submitted",
      duplicateOfRevisionId: null,
      supersededRevisionIds: reviewCandidate.supersededRevisions.map((item) => item.id)
    };
  });
}

export async function processOwnerAction({ revisionId, actorUserId, action, comment }) {
  const revision = await findRevisionById(revisionId);

  if (!revision || revision.state !== "review") {
    throw new Error("Только версии на проверке могут получать решения владельца.");
  }

  const entity = await findEntityById(revision.entityId);

  if (action === "approve") {
    const updated = await updateRevision(revisionId, {
      ownerApprovalStatus: "approved",
      ownerApprovedBy: actorUserId,
      ownerApprovedAt: new Date(),
      reviewComment: comment ?? "",
      userId: actorUserId
    });

    await recordAuditEvent({
      entityId: entity.id,
      revisionId,
      actorUserId,
      eventKey: AUDIT_EVENT_KEYS.OWNER_APPROVED,
      summary: "Владелец согласовал версию.",
      details: buildReviewAuditDetails(revision, {
        comment: comment ?? ""
      })
    });

    return updated;
  }

  const eventKey = action === "reject" ? AUDIT_EVENT_KEYS.OWNER_REJECTED : AUDIT_EVENT_KEYS.SENT_BACK_WITH_COMMENT;
  const summary = action === "reject"
    ? "Владелец отклонил версию."
    : "Версия возвращена с комментарием.";

  const updated = await updateRevision(revisionId, {
    state: "draft",
    ownerApprovalStatus: "rejected",
    reviewComment: comment ?? "",
    userId: actorUserId
  });

  await recordAuditEvent({
    entityId: entity.id,
    revisionId,
    actorUserId,
    eventKey,
    summary,
    details: buildReviewAuditDetails(revision, {
      comment: comment ?? ""
    })
  });

  return updated;
}

function needsSlugObligations(entityType, previousPayload, nextPayload) {
  return hasSlugMutation(entityType, previousPayload, nextPayload);
}

export async function publishRevision({ revisionId, actorUserId }) {
  const revision = await findRevisionById(revisionId);

  if (!revision || revision.state !== "review") {
    throw new Error("Только версии на проверке можно публиковать.");
  }

  const entity = await findEntityById(revision.entityId);
  const globalSettingsRevision = entity.entityType === ENTITY_TYPES.GLOBAL_SETTINGS
    ? null
    : await getPublishedGlobalSettingsRevision();
  const readiness = await evaluateReadiness({ entity, revision, globalSettingsRevision });

  if (readiness.hasBlocking) {
    await recordAuditEvent({
      entityId: entity.id,
      revisionId,
      actorUserId,
      eventKey: AUDIT_EVENT_KEYS.PUBLISH_BLOCKED,
      summary: "Публикация заблокирована проверкой готовности.",
      details: {
        readiness
      }
    });

    throw new Error("Публикация заблокирована проверкой готовности.");
  }

  if (revision.ownerApprovalStatus !== "approved") {
    throw new Error("Не хватает обязательного согласования владельца.");
  }

  if (revision.previewStatus !== PREVIEW_STATUS.RENDERABLE) {
    throw new Error("Перед публикацией предпросмотр должен быть доступен.");
  }

  if (entity.entityType === ENTITY_TYPES.SERVICE || entity.entityType === ENTITY_TYPES.CASE) {
    const collision = await findEntityByPublishedSlugCollision(entity.entityType, revision.payload.slug, entity.id);

    if (collision) {
      throw new Error("Публикацию блокирует конфликт короткого адреса.");
    }
  }

  const previousPublishedRevision = await findActivePublishedRevision(entity.id);

  const publishedRevision = await updateRevision(revisionId, {
    state: "published",
    publishedAt: new Date(),
    publishedBy: actorUserId,
    userId: actorUserId
  });

  await updateEntityTimestamps(entity.id, actorUserId, revisionId);

  const createdObligationTypes = [];

  if (needsSlugObligations(entity.entityType, previousPublishedRevision?.payload, publishedRevision.payload)) {
    for (const obligationType of [
      "redirect_required",
      "revalidation_required",
      "sitemap_update_required",
      "canonical_url_check_required"
    ]) {
      await insertPublishObligation({
        entityId: entity.id,
        revisionId,
        obligationType,
        payload: {
          previousSlug: previousPublishedRevision.payload.slug,
          nextSlug: publishedRevision.payload.slug
        }
      });
      createdObligationTypes.push(obligationType);
    }

    await recordAuditEvent({
      entityId: entity.id,
      revisionId,
      actorUserId: null,
      eventKey: AUDIT_EVENT_KEYS.SLUG_CHANGE_OBLIGATION_CREATED,
      summary: "Созданы обязательства после смены адреса.",
      details: {
        previousSlug: previousPublishedRevision.payload.slug,
        nextSlug: publishedRevision.payload.slug
      }
    });
  }

  await recordAuditEvent({
    entityId: entity.id,
    revisionId,
    actorUserId,
    eventKey: AUDIT_EVENT_KEYS.PUBLISHED,
    summary: "Версия опубликована.",
    details: {
      readiness
    }
  });

  const publishFollowUp = buildPublishFollowUp({
    entityType: entity.entityType,
    previousPayload: previousPublishedRevision?.payload ?? null,
    nextPayload: publishedRevision.payload,
    obligationTypes: createdObligationTypes
  });

  return {
    entity,
    revision: publishedRevision,
    readiness,
    publishFollowUp
  };
}

export async function publishGalleryCollectionRevision({ revisionId, actorUserId }) {
  const revision = await findRevisionById(revisionId);

  if (!revision || revision.state !== "draft") {
    throw new Error("Только черновик коллекции можно опубликовать напрямую.");
  }

  const entity = await findEntityById(revision.entityId);

  if (!entity || entity.entityType !== ENTITY_TYPES.GALLERY) {
    throw new Error("Прямая публикация доступна только для коллекций.");
  }

  if (entity.markedForRemovalAt) {
    const error = new Error("Нельзя публиковать коллекцию, помеченную на удаление.");
    error.code = "COLLECTION_PUBLISH_BLOCKED";
    throw error;
  }

  const globalSettingsRevision = await getPublishedGlobalSettingsRevision();
  const readiness = await evaluateReadiness({ entity, revision, globalSettingsRevision });

  if (readiness.hasBlocking) {
    await recordAuditEvent({
      entityId: entity.id,
      revisionId,
      actorUserId,
      eventKey: AUDIT_EVENT_KEYS.PUBLISH_BLOCKED,
      summary: "Публикация коллекции заблокирована проверкой готовности.",
      details: {
        readiness,
        directGalleryPublish: true
      }
    });

    const error = new Error("Публикация коллекции заблокирована проверкой готовности.");
    error.code = "COLLECTION_PUBLISH_BLOCKED";
    throw error;
  }

  if (revision.previewStatus !== PREVIEW_STATUS.RENDERABLE) {
    const error = new Error("Перед публикацией коллекции предпросмотр должен быть доступен.");
    error.code = "COLLECTION_PUBLISH_BLOCKED";
    throw error;
  }

  const previousPublishedRevision = await findActivePublishedRevision(entity.id);

  const publishedRevision = await updateRevision(revisionId, {
    state: "published",
    publishedAt: new Date(),
    publishedBy: actorUserId,
    ownerReviewRequired: false,
    ownerApprovalStatus: "not_required",
    userId: actorUserId
  });

  await updateEntityTimestamps(entity.id, actorUserId, revisionId);

  await recordAuditEvent({
    entityId: entity.id,
    revisionId,
    actorUserId,
    eventKey: AUDIT_EVENT_KEYS.PUBLISHED,
    summary: "Коллекция опубликована без повторного согласования владельцем.",
    details: {
      readiness,
      directGalleryPublish: true
    }
  });

  const publishFollowUp = buildPublishFollowUp({
    entityType: entity.entityType,
    previousPayload: previousPublishedRevision?.payload ?? null,
    nextPayload: publishedRevision.payload,
    obligationTypes: []
  });

  return {
    entity,
    revision: publishedRevision,
    readiness,
    publishFollowUp
  };
}

export async function rollbackPublishedEntity({ entityId, targetRevisionId, actorUserId }) {
  const entity = await findEntityById(entityId);
  const targetRevision = await findRevisionById(targetRevisionId);

  if (!entity || !targetRevision || targetRevision.entityId !== entityId || targetRevision.state !== "published") {
    throw new Error("Для отката нужна опубликованная версия этой же сущности.");
  }

  await updateEntityTimestamps(entity.id, actorUserId, targetRevisionId);

  await recordAuditEvent({
    entityId,
    revisionId: targetRevisionId,
    actorUserId,
    eventKey: AUDIT_EVENT_KEYS.ROLLBACK_EXECUTED,
    summary: "Откат вернул предыдущую опубликованную версию.",
    details: {}
  });

  return {
    entity,
    targetRevision
  };
}

export async function completePublishObligation(obligationId) {
  const obligation = await findPublishObligationById(obligationId);

  if (!obligation) {
    throw new Error("Обязательство публикации не найдено.");
  }

  await markPublishObligationCompleted(obligationId);

  const entity = await findEntityById(obligation.entityId);
  const revision = obligation.revisionId ? await findRevisionById(obligation.revisionId) : null;
  const previousSlug = obligation?.payload?.previousSlug;
  const publishFollowUp = buildPublishFollowUp({
    entityType: entity?.entityType,
    previousPayload: previousSlug ? { slug: previousSlug } : null,
    nextPayload: revision?.payload ?? null,
    obligationTypes: [obligation.obligationType]
  });

  return {
    obligation,
    entity,
    revision,
    publishFollowUp
  };
}
