...
export async function recordDestructiveEvent(input, options = {}) {
  const auditWriter = options.recordAuditEvent ?? recordAuditEvent;
  const forensicWriter = options.insertDestructiveForensicEvent ?? insertDestructiveForensicEvent;
  ...
  const auditEntityId = Object.prototype.hasOwnProperty.call(input, "auditEntityId")
    ? normalizeString(input.auditEntityId) || null
    : target.entityId;
